/**
 * mediaOriginals.ts
 *
 * メディア横断原作逆引きツール用データ定義。
 * 映像作品（映画・ドラマ・アニメ・海外）と、その原作書籍の対応表。
 */

export type MediaType = "movie" | "drama" | "anime" | "overseas";
export type OriginalType = "novel" | "manga" | "nonfiction" | "essay";

export type MediaOriginalItem = {
  id: string;
  /** 映像作品名 */
  mediaTitle: string;
  /** メディア種別 */
  mediaType: MediaType;
  /** 公開・放送年 */
  mediaYear?: number;
  /** 原作が存在するか */
  originalExists: boolean;
  /** 原作タイトル */
  originalTitle?: string;
  /** 原作著者 */
  originalAuthor?: string;
  /** 原作種別 */
  originalType?: OriginalType;
  /** 映像化の形式（例: テレビドラマ化・映画化） */
  adaptationLabel?: string;
  /** 一言説明 */
  description: string;
  /** 検索用別名・表記揺れ */
  searchAliases?: string[];
  /** 原作と映像の違い（モーダル詳細用） */
  adaptationNotes?: string;
  /** おすすめ対象（モーダル詳細用） */
  recommendedFor?: string;
  /** /works/[workId] に対応するID（存在する場合のみ） */
  workId?: string;
  /** 原作書籍のサムネイルURL */
  thumbnailUrl?: string;
};

export const MEDIA_ORIGINALS: MediaOriginalItem[] = [
  {
    id: "galileo-drama",
    mediaTitle: "ガリレオ",
    mediaType: "drama",
    mediaYear: 2007,
    originalExists: true,
    originalTitle: "探偵ガリレオ",
    originalAuthor: "東野圭吾",
    originalType: "novel",
    adaptationLabel: "テレビドラマ化",
    description:
      "天才物理学者・湯川学が難事件を解決する人気推理ドラマ。フジテレビ系。原作は1998年刊の連作短編集。",
    searchAliases: ["探偵ガリレオ", "湯川学", "東野圭吾", "福山雅治"],
    adaptationNotes:
      "ドラマではオリジナルエピソードや女性刑事・内海薫のキャラクターが加えられている。原作は短編中心で、ドラマよりもロジックの密度が高い。",
    recommendedFor:
      "ドラマを見て東野圭吾の世界観に興味を持った人。科学的トリックと人間ドラマのバランスが好きな人。",
    workId: "05gq12q",
  thumbnailUrl: "https://books.google.com/books/content?id=X2s-AgAACAAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "suspect-x",
    mediaTitle: "容疑者Xの献身",
    mediaType: "movie",
    mediaYear: 2008,
    originalExists: true,
    originalTitle: "容疑者Xの献身",
    originalAuthor: "東野圭吾",
    originalType: "novel",
    adaptationLabel: "映画化",
    description:
      "天才数学者の犯罪と湯川の推理が交わるガリレオシリーズ映画版。直木賞・本格ミステリ大賞受賞の原作小説。",
    searchAliases: ["ガリレオ", "東野圭吾", "容疑者X"],
    adaptationNotes:
      "映画は概ね原作に忠実。ラストシーンの演出に若干の変更あり。原作の方が石神（犯人）の心理描写が深い。",
    recommendedFor:
      "映画で感動した人。東野圭吾ミステリーで「犯人側の視点」を楽しみたい人。",
    workId: "0bkks71",
  thumbnailUrl: "https://books.google.com/books/content?id=eNjdDwAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "nodame-drama",
    mediaTitle: "のだめカンタービレ",
    mediaType: "drama",
    mediaYear: 2006,
    originalExists: true,
    originalTitle: "のだめカンタービレ",
    originalAuthor: "二ノ宮知子",
    originalType: "manga",
    adaptationLabel: "テレビドラマ化",
    description:
      "音楽大学を舞台にしたラブコメディ。Kiss連載の漫画原作。クラシック音楽ブームのきっかけとなった人気作。",
    searchAliases: ["のだめ", "二ノ宮知子", "千秋先輩"],
    adaptationNotes:
      "ドラマは漫画のトーンをよく再現。一部エピソードが省略・再構成されている。原作はパリ編・ウィーン編がさらに続く。",
    recommendedFor:
      "音楽・クラシックに興味を持った人。ラブコメとして楽しみたい人。",
    workId: "1kx24fd",
  thumbnailUrl: "https://books.google.com/books/content?id=GZpnEAAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "nodame-anime",
    mediaTitle: "のだめカンタービレ（アニメ）",
    mediaType: "anime",
    mediaYear: 2007,
    originalExists: true,
    originalTitle: "のだめカンタービレ",
    originalAuthor: "二ノ宮知子",
    originalType: "manga",
    adaptationLabel: "アニメ化",
    description:
      "同名漫画のTVアニメ版。J.C.STAFF制作。原作の雰囲気を保ちながら漫画的テンポで描く。",
    searchAliases: ["のだめ", "二ノ宮知子"],
    workId: "1kx24fd",
  thumbnailUrl: "https://books.google.com/books/content?id=GZpnEAAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "death-note-anime",
    mediaTitle: "DEATH NOTE",
    mediaType: "anime",
    mediaYear: 2006,
    originalExists: true,
    originalTitle: "DEATH NOTE",
    originalAuthor: "大場つぐみ・小畑健",
    originalType: "manga",
    adaptationLabel: "アニメ化",
    description:
      "名前を書いた人間を死なせるノートを手にした高校生の心理戦ミステリー。週刊少年ジャンプ連載漫画のアニメ化。",
    searchAliases: ["デスノート", "大場つぐみ", "小畑健", "ライト", "L"],
    adaptationNotes:
      "アニメ版は原作に概ね忠実だが、後半の展開に変更がある（Nとの対決など）。原作漫画の方が心理描写のテンポが速い。",
    recommendedFor:
      "心理戦・頭脳戦が好きな人。善悪の境界を問う倫理的テーマに興味がある人。",
    workId: "1m22mge",
  thumbnailUrl: "https://books.google.com/books/content?id=_ERuCwAAQBAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "death-note-movie",
    mediaTitle: "DEATH NOTE（実写映画）",
    mediaType: "movie",
    mediaYear: 2006,
    originalExists: true,
    originalTitle: "DEATH NOTE",
    originalAuthor: "大場つぐみ・小畑健",
    originalType: "manga",
    adaptationLabel: "映画化",
    description:
      "同名漫画の実写映画版。藤原竜也・松山ケンイチ主演。前後編2作構成。",
    searchAliases: ["デスノート", "大場つぐみ", "小畑健"],
    workId: "1m22mge",
  thumbnailUrl: "https://books.google.com/books/content?id=_ERuCwAAQBAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "kimi-no-na-wa",
    mediaTitle: "君の名は。",
    mediaType: "movie",
    mediaYear: 2016,
    originalExists: true,
    originalTitle: "君の名は。",
    originalAuthor: "新海誠",
    originalType: "novel",
    adaptationLabel: "映画→ノベライズ",
    description:
      "新海誠監督のアニメーション映画。映画公開とほぼ同時にノベライズが刊行。国内興収250億円超の大ヒット作。",
    searchAliases: ["きみのなは", "新海誠", "ほだか", "みつは"],
    adaptationNotes:
      "小説は映画と同内容だが、心理描写・情景描写が文章で丁寧に補完されている。映画を見た後に読むと新たな発見がある。",
    recommendedFor:
      "映画を見て余韻に浸りたい人。新海誠ワールドを活字で体験したい人。",
    workId: "045cnn5",
  thumbnailUrl: "https://books.google.com/books/content?id=aYP2vwEACAAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "chihayafuru-anime",
    mediaTitle: "ちはやふる",
    mediaType: "anime",
    mediaYear: 2011,
    originalExists: true,
    originalTitle: "ちはやふる",
    originalAuthor: "末次由紀",
    originalType: "manga",
    adaptationLabel: "アニメ化",
    description:
      "競技かるたに青春をかける少女を描く漫画のアニメ版。マドハウス制作。競技かるたの認知を大きく広めた。",
    searchAliases: ["末次由紀", "かるた", "ちはやふる"],
    adaptationNotes:
      "アニメは原作の雰囲気を高水準で再現。漫画は全50巻で、アニメよりさらに先まで物語が続く。",
    recommendedFor:
      "アニメで感動した人。百人一首・競技かるたに興味を持った人。",
    workId: "16a2phy",
  thumbnailUrl: "https://books.google.com/books/content?id=GVNODwAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "chihayafuru-movie",
    mediaTitle: "ちはやふる（実写映画）",
    mediaType: "movie",
    mediaYear: 2016,
    originalExists: true,
    originalTitle: "ちはやふる",
    originalAuthor: "末次由紀",
    originalType: "manga",
    adaptationLabel: "映画化",
    description:
      "同名漫画の実写映画版。広瀬すず・野村周平主演。上の句・下の句・結び の3部作。",
    searchAliases: ["末次由紀", "かるた", "広瀬すず"],
    workId: "16a2phy",
  thumbnailUrl: "https://books.google.com/books/content?id=GVNODwAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "toshokan-senso",
    mediaTitle: "図書館戦争",
    mediaType: "movie",
    mediaYear: 2013,
    originalExists: true,
    originalTitle: "図書館戦争",
    originalAuthor: "有川浩",
    originalType: "novel",
    adaptationLabel: "映画化",
    description:
      "メディア規制に立ち向かう図書館防衛隊を描くラブストーリー＆アクション。岡田准一・榮倉奈々主演。",
    searchAliases: ["有川浩", "有川ひろ", "としょかんせんそう"],
    adaptationNotes:
      "映画は原作をかなり圧縮。原作シリーズは全4巻＋外伝があり、恋愛描写がより丁寧に描かれている。",
    recommendedFor:
      "映画でキャラクターが好きになった人。言論の自由・検閲というテーマに興味がある人。",
    workId: "05vxbou",
  thumbnailUrl: "https://books.google.com/books/content?id=q1imYpx_hbUC&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "hakuyako-drama",
    mediaTitle: "白夜行",
    mediaType: "drama",
    mediaYear: 2006,
    originalExists: true,
    originalTitle: "白夜行",
    originalAuthor: "東野圭吾",
    originalType: "novel",
    adaptationLabel: "テレビドラマ化",
    description:
      "1973年の殺人事件から始まる二人の男女の長い物語。TBS系。山田孝之・綾瀬はるか主演。",
    searchAliases: ["東野圭吾", "はくやこう", "白夜行"],
    adaptationNotes:
      "ドラマと原作でラストが異なる。原作は「描かれない」ことで成立する構造が特徴で、より謎が深い。",
    recommendedFor:
      "ドラマで感動した人。東野圭吾の暗黒面・社会派ミステリーが好きな人。",
    workId: "1tzlddg",
  thumbnailUrl: "https://books.google.com/books/content?id=KZgLEAAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "kokuhaku-movie",
    mediaTitle: "告白",
    mediaType: "movie",
    mediaYear: 2010,
    originalExists: true,
    originalTitle: "告白",
    originalAuthor: "湊かなえ",
    originalType: "novel",
    adaptationLabel: "映画化",
    description:
      "子供を殺された女性教師の復讐を描く心理サスペンス映画。松たか子主演。本屋大賞受賞の原作小説。",
    searchAliases: ["湊かなえ", "みなとかなえ", "告白"],
    adaptationNotes:
      "映画は原作の複数視点構造を映像的に高度に再現。中島哲也監督の映像表現が原作の世界観をさらに深めている。",
    recommendedFor:
      "映画の衝撃に圧倒された人。湊かなえのイヤミスジャンルを知りたい人。",
    workId: "0kn9l8z",
  thumbnailUrl: "https://books.google.com/books/content?id=-V6YzwEACAAJ&printsec=frontcover&img=1&zoom=1&imgtk=AFLRE70ryxsYTDYDZugfH6AcBU1msbde1jggv_LeTIO-raQ2hh_eNsa4WQX_5f5u_ugemP8OSlLMiGsUDKZomB396C9WtWALT8Tq5tbAdbvsQ2zYI8Af2WbfFYvyghqPMmVjoY31Blyy&source=gbs_api",
  },
  {
    id: "harry-potter",
    mediaTitle: "ハリー・ポッター",
    mediaType: "overseas",
    mediaYear: 2001,
    originalExists: true,
    originalTitle: "ハリー・ポッターと賢者の石",
    originalAuthor: "J.K.ローリング",
    originalType: "novel",
    adaptationLabel: "映画化（海外）",
    description:
      "魔法学校を舞台にした世界的ベストセラー小説の映画版。全8作。世界累計5億部を超える原作小説。",
    searchAliases: ["ハリポタ", "J.K.ローリング", "ホグワーツ", "ハーミオーニー"],
    adaptationNotes:
      "映画は各巻を1〜2本に圧縮しており、原作のエピソードが多数省略されている。原作の方が世界観と人物描写が圧倒的に豊か。",
    recommendedFor:
      "映画シリーズを楽しんだ人。子供から大人まで楽しめるファンタジーを読みたい人。",
    workId: "1rmuzaj",
  thumbnailUrl: "https://books.google.com/books/content?id=-g-O0AEACAAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "kimetsu",
    mediaTitle: "鬼滅の刃",
    mediaType: "anime",
    mediaYear: 2019,
    originalExists: true,
    originalTitle: "鬼滅の刃",
    originalAuthor: "吾峠呼世晴",
    originalType: "manga",
    adaptationLabel: "アニメ化",
    description:
      "大正時代を舞台にした鬼狩りの物語。ufotable制作のアニメが爆発的ヒット。週刊少年ジャンプ連載。",
    searchAliases: ["きめつのやいば", "吾峠呼世晴", "炭治郎"],
    adaptationNotes:
      "アニメは原作に非常に忠実で、映像・音楽のクオリティが高く評価されている。原作漫画は全23巻で完結済み。",
    recommendedFor:
      "アニメを見てもっと世界観を知りたい人。大正時代の設定と鬼との戦いを活字で楽しみたい人。",
    workId: "0iuwa8r",
  thumbnailUrl: "https://books.google.com/books/content?id=Mj1cDAAAQBAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "shingeki",
    mediaTitle: "進撃の巨人",
    mediaType: "anime",
    mediaYear: 2013,
    originalExists: true,
    originalTitle: "進撃の巨人",
    originalAuthor: "諫山創",
    originalType: "manga",
    adaptationLabel: "アニメ化",
    description:
      "巨人に支配された世界を描く壮大なアクション・ミステリー。別冊少年マガジン連載。WIT STUDIO/MAPPA制作。",
    searchAliases: ["しんげき", "諫山創", "エレン", "リヴァイ"],
    adaptationNotes:
      "アニメは原作に忠実だが、制作スタジオが途中で変更されるため画風が変わる。原作漫画全34巻で完結済み。",
    recommendedFor:
      "アニメの謎と世界観に引き込まれた人。伏線・考察が好きな人。",
    workId: "0zni0pn",
  thumbnailUrl: "https://books.google.com/books/content?id=nMjEBQAAQBAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "hanzawa-drama",
    mediaTitle: "半沢直樹",
    mediaType: "drama",
    mediaYear: 2013,
    originalExists: true,
    originalTitle: "オレたちバブル入行組",
    originalAuthor: "池井戸潤",
    originalType: "novel",
    adaptationLabel: "テレビドラマ化",
    description:
      "銀行員・半沢直樹が組織の不正に立ち向かうビジネスドラマ。TBS系。最高視聴率42.2%を記録した社会現象。",
    searchAliases: ["池井戸潤", "バブル入行組", "倍返し", "半沢"],
    adaptationNotes:
      "原作のタイトルは「オレたちバブル入行組」で、ドラマが半沢直樹の名を冠して独自に展開。続編シリーズも小説・ドラマで展開している。",
    recommendedFor:
      "ドラマで痛快なカタルシスを味わった人。組織の不正に立ち向かう社会派小説が好きな人。",
    workId: "0r02lo2",
  thumbnailUrl: "https://books.google.com/books/content?id=ITHyAAAAMAAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "shitamachi-rocket",
    mediaTitle: "下町ロケット",
    mediaType: "drama",
    mediaYear: 2015,
    originalExists: true,
    originalTitle: "下町ロケット",
    originalAuthor: "池井戸潤",
    originalType: "novel",
    adaptationLabel: "テレビドラマ化",
    description:
      "中小企業の社長が夢を追いかけるビジネスドラマ。TBS系。直木賞受賞の原作小説。阿部寛主演。",
    searchAliases: ["池井戸潤", "下町ロケット"],
    adaptationNotes:
      "ドラマは原作に概ね忠実。シーズン2（ゴースト/ヤタガラス）はドラマ独自のストーリー展開が増える。",
    recommendedFor:
      "ものづくりと組織の葛藤を描いた物語が好きな人。池井戸潤のビジネス小説に入門したい人。",
    workId: "0cy5usk",
  thumbnailUrl: "https://books.google.com/books/content?id=bG-DDAAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "dragon-zakura",
    mediaTitle: "ドラゴン桜",
    mediaType: "drama",
    mediaYear: 2005,
    originalExists: true,
    originalTitle: "ドラゴン桜",
    originalAuthor: "三田紀房",
    originalType: "manga",
    adaptationLabel: "テレビドラマ化",
    description:
      "落ちこぼれ高校生を東大合格させようとする弁護士の物語。TBS系。2021年に続編ドラマも放送。",
    searchAliases: ["三田紀房", "東大", "ドラゴン桜2", "阿部寛"],
    workId: "0qseeqa",
  thumbnailUrl: "https://books.google.com/books/content?id=ncVYzgEACAAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "hakase-no-ai",
    mediaTitle: "博士の愛した数式",
    mediaType: "movie",
    mediaYear: 2006,
    originalExists: true,
    originalTitle: "博士の愛した数式",
    originalAuthor: "小川洋子",
    originalType: "novel",
    adaptationLabel: "映画化",
    description:
      "80分しか記憶が続かない博士と家政婦の交流を描く温かい物語。寺尾聰主演。読売文学賞・本屋大賞受賞作。",
    searchAliases: ["小川洋子", "数式", "博士"],
    adaptationNotes:
      "映画は原作に忠実で、文学的な余韻も丁寧に再現。原作の方が数学への愛情と人物の内面描写が豊かで深い。",
    recommendedFor:
      "映画で涙した人。静かで知的な文学作品を読みたい人。数学と人間の関わりに興味がある人。",
    workId: "004w4z3",
  thumbnailUrl: "https://books.google.com/books/content?id=vYbDBQAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "alice-in-borderland",
    mediaTitle: "今際の国のアリス",
    mediaType: "drama",
    mediaYear: 2020,
    originalExists: true,
    originalTitle: "今際の国のアリス",
    originalAuthor: "麻生羽呂",
    originalType: "manga",
    adaptationLabel: "Netflix実写化",
    description:
      "謎のゲームに巻き込まれる若者たちのサバイバル。Netflixオリジナルドラマ。週刊少年サンデーS連載漫画が原作。",
    searchAliases: ["アリスインボーダーランド", "麻生羽呂", "今際"],
    adaptationNotes:
      "Netflixドラマは原作のゲーム・キャラクターを一部変更・再構成。原作漫画は全18巻でより緻密なゲームルール設計がされている。",
    recommendedFor:
      "ドラマのサバイバル展開にハマった人。デスゲーム・謎解きジャンルが好きな人。",
    workId: "0lu6cg1",
  thumbnailUrl: "https://books.google.com/books/content?id=4kMrtAEACAAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "suna-no-utsuwa",
    mediaTitle: "砂の器",
    mediaType: "movie",
    mediaYear: 1974,
    originalExists: true,
    originalTitle: "砂の器",
    originalAuthor: "松本清張",
    originalType: "novel",
    adaptationLabel: "映画化",
    description:
      "殺人事件と人物の壮絶な過去が交差する社会派ミステリー。野村芳太郎監督。日本映画史に残る名作映画。",
    searchAliases: ["松本清張", "すなのうつわ"],
    adaptationNotes:
      "映画はラストシーンの音楽演奏と回想の交差が特に高く評価される。原作小説はより社会批評的な視点が強く、事件の社会背景が詳細。",
    recommendedFor:
      "映画を見て松本清張に興味を持った人。社会派ミステリーに入門したい人。",
    workId: "1dem5jj",
  thumbnailUrl: "https://books.google.com/books/content?id=1WAsAQAAIAAJ&printsec=frontcover&img=1&zoom=1&imgtk=AFLRE706MT_6rwnodk1OVZqKqAsHJGS0wFu7pd2M1pWFKZqsULVGcj0bxG8kdZNdqj7XHFnzgMO9-00GE-Vpi0E_Si9tXmeadllWEIKcmKzgPDEeDuuplrBT4v1n1cVKsc_K-xQIvQr1&source=gbs_api",
  },
  {
    id: "64-movie",
    mediaTitle: "64-ロクヨン-",
    mediaType: "movie",
    mediaYear: 2016,
    originalExists: true,
    originalTitle: "64（ロクヨン）",
    originalAuthor: "横山秀夫",
    originalType: "novel",
    adaptationLabel: "映画化",
    description:
      "昭和64年の誘拐事件が現代に再び動き出す警察ミステリー。佐藤浩市主演。前後編映画。",
    searchAliases: ["横山秀夫", "ろくよん", "64"],
    adaptationNotes:
      "映画は原作を前後編に圧縮しており、警察組織内部の権力闘争の細部が省略されている。原作の方が組織小説として読み応えが高い。",
    recommendedFor:
      "映画を見て横山秀夫に興味を持った人。警察組織の内部を描く社会派ミステリーが好きな人。",
    workId: "1qznmx7",
  thumbnailUrl: "https://books.google.com/books/content?id=cTTBMgEACAAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "kotonoha-no-niwa",
    mediaTitle: "言の葉の庭",
    mediaType: "movie",
    mediaYear: 2013,
    originalExists: true,
    originalTitle: "言の葉の庭",
    originalAuthor: "新海誠",
    originalType: "novel",
    adaptationLabel: "映画→ノベライズ",
    description:
      "新海誠監督の短編アニメーション映画（46分）。雨の日の公園で出会う男女の物語。映画公開後にノベライズが刊行。",
    searchAliases: ["新海誠", "ことのはのにわ"],
    adaptationNotes:
      "小説では映画で描かれない側の登場人物の心理・背景が補完されており、映画の後に読むと新たな感動がある。",
    recommendedFor:
      "新海誠の映像美に感動した人。短時間で感動体験したい人。",
    workId: "1shuw72",
  thumbnailUrl: "https://books.google.com/books/content?id=mi8hwAEACAAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "joker-game",
    mediaTitle: "ジョーカー・ゲーム",
    mediaType: "movie",
    mediaYear: 2015,
    originalExists: true,
    originalTitle: "ジョーカー・ゲーム",
    originalAuthor: "柳広司",
    originalType: "novel",
    adaptationLabel: "映画化",
    description:
      "第二次世界大戦前後を舞台にしたスパイ小説の映画化。亀梨和也主演。直木賞候補の原作連作短編集。",
    searchAliases: ["柳広司", "スパイ", "D機関"],
    workId: "1mij96r",
  thumbnailUrl: "https://books.google.com/books/content?id=UiiOPgAACAAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "eizo-ken",
    mediaTitle: "映像研には手を出すな！",
    mediaType: "anime",
    mediaYear: 2020,
    originalExists: true,
    originalTitle: "映像研には手を出すな！",
    originalAuthor: "大童澄瞳",
    originalType: "manga",
    adaptationLabel: "アニメ化",
    description:
      "高校のアニメ部を舞台に映像制作に情熱を注ぐ少女たちの物語。Science SARU制作。湯浅政明監督。",
    searchAliases: ["映像研", "大童澄瞳"],
    adaptationNotes:
      "アニメはアニメーション表現そのものへのメタ的な演出が特に高く評価される。原作漫画もアニメーション制作の現場描写が詳細。",
    recommendedFor:
      "アニメ制作・創作に興味がある人。青春と情熱を描いた作品が好きな人。",
    workId: "1cv7fb3",
  thumbnailUrl: "https://books.google.com/books/content?id=7qHvDQAAQAAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },

  // ── 2000年代 ────────────────────────────────────────────────────

  {
    id: "hana-yori-dango",
    mediaTitle: "花より男子",
    mediaType: "drama",
    mediaYear: 2005,
    originalExists: true,
    originalTitle: "花より男子",
    originalAuthor: "神尾葉子",
    originalType: "manga",
    adaptationLabel: "テレビドラマ化",
    description:
      "平凡な少女が花の御堂4グループ（F4）と衝突するラブストーリー。TBS系。松本潤・井上真央主演。アジア全域でリメイクされた人気漫画原作。",
    searchAliases: ["神尾葉子", "F4", "道明寺", "つくし", "ハナダン"],
    adaptationNotes:
      "ドラマは原作の長大なストーリーを大幅に圧縮。原作漫画は全36巻で、ドラマで描かれないエピソードや登場人物が多数ある。",
    recommendedFor:
      "ドラマで恋愛展開にドキドキした人。王道少女漫画の原点を体験したい人。",
    workId: "1v5wh1g",
  thumbnailUrl: "https://books.google.com/books/content?id=oaQUAQAAMAAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "rookies-drama",
    mediaTitle: "ROOKIES",
    mediaType: "drama",
    mediaYear: 2008,
    originalExists: true,
    originalTitle: "ROOKIES",
    originalAuthor: "森田まさのり",
    originalType: "manga",
    adaptationLabel: "テレビドラマ化",
    description:
      "問題校の野球部を立て直す熱血教師の物語。TBS系。佐藤隆太主演。甲子園を目指す高校生の青春を描く。週刊少年ジャンプ連載漫画が原作。",
    searchAliases: ["森田まさのり", "ルーキーズ", "甲子園", "川藤"],
    adaptationNotes:
      "ドラマは原作の前半〜中盤を主に映像化。漫画は全24巻で、甲子園出場後の展開もしっかり描かれている。",
    recommendedFor:
      "ドラマで青春の熱さに感動した人。師弟関係と不良の更生をテーマにした漫画が好きな人。",
    workId: "0hy0c7e",
  thumbnailUrl: "https://books.google.com/books/content?id=gq7ICwAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "hachi-kuro-anime",
    mediaTitle: "ハチミツとクローバー",
    mediaType: "anime",
    mediaYear: 2005,
    originalExists: true,
    originalTitle: "ハチミツとクローバー",
    originalAuthor: "羽海野チカ",
    originalType: "manga",
    adaptationLabel: "アニメ化",
    description:
      "美大生たちの青春と恋愛と挫折を描く群像劇。J.C.STAFF制作。ヤングユーコミックス連載。後に実写映画化・実写ドラマ化も。",
    searchAliases: ["羽海野チカ", "ハチクロ", "竹本", "花本"],
    adaptationNotes:
      "アニメは原作の雰囲気をよく再現。漫画は全10巻で、アニメよりも内面描写が丁寧で余韻が深い。",
    recommendedFor:
      "アニメで芸術・青春・一方通行の恋のテーマに共感した人。3月のライオンも好きな人。",
    workId: "1136xxp",
  thumbnailUrl: "https://books.google.com/books/content?id=WKGADwAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "nana-anime",
    mediaTitle: "NANA",
    mediaType: "anime",
    mediaYear: 2006,
    originalExists: true,
    originalTitle: "NANA",
    originalAuthor: "矢沢あい",
    originalType: "manga",
    adaptationLabel: "アニメ化",
    description:
      "同じ名前の対照的な二人の「ナナ」の友情と恋愛を描く人気少女漫画のアニメ版。マッドハウス制作。Cookies連載。",
    searchAliases: ["矢沢あい", "ナナ", "ブラスト", "大崎ナナ", "小松奈々"],
    adaptationNotes:
      "アニメは原作の連載中断までの内容を丁寧に映像化。漫画は2009年から休載中のため、物語の続きはアニメでも未完。",
    recommendedFor:
      "アニメで二人のナナの友情に感動した人。音楽と恋愛と友情をテーマにした少女漫画が好きな人。",
    workId: "0xg4crk",
  thumbnailUrl: "https://books.google.com/books/content?id=eNzICwAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "kimi-ni-todoke-anime",
    mediaTitle: "君に届け",
    mediaType: "anime",
    mediaYear: 2009,
    originalExists: true,
    originalTitle: "君に届け",
    originalAuthor: "椎名軽穂",
    originalType: "manga",
    adaptationLabel: "アニメ化",
    description:
      "陰気と思われていた少女・爽子と人気者・風早の純粋な恋愛を描く少女漫画のアニメ版。Production I.G制作。週刊別マーガレット連載。",
    searchAliases: ["椎名軽穂", "爽子", "風早", "くらのすけ"],
    adaptationNotes:
      "アニメは原作の序盤〜中盤を映像化。漫画全30巻で描かれる二人の関係の進展はアニメよりもはるかに長い。",
    recommendedFor:
      "アニメで純粋な恋愛模様に癒された人。不器用な女の子が成長する王道少女漫画が好きな人。",
    workId: "1d6gduo",
  thumbnailUrl: "https://books.google.com/books/content?id=b88vAQAAIAAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "fma-brotherhood",
    mediaTitle: "鋼の錬金術師 BROTHERHOOD",
    mediaType: "anime",
    mediaYear: 2009,
    originalExists: true,
    originalTitle: "鋼の錬金術師",
    originalAuthor: "荒川弘",
    originalType: "manga",
    adaptationLabel: "アニメ化（完全版）",
    description:
      "錬金術師の兄弟が「賢者の石」を求めて旅する壮大な冒険物語。BONES制作。月刊少年ガンガン連載原作を完全映像化。",
    searchAliases: ["荒川弘", "エドワード", "アルフォンス", "錬金術師", "FMA"],
    adaptationNotes:
      "BROTHERHOODは2003年版と異なり、原作漫画全27巻を忠実に映像化。アニメ史に残る高完成度の作品。原作漫画は伏線の密度と戦略的な戦闘描写が特徴。",
    recommendedFor:
      "アニメの完成度の高い物語に感動した人。「等価交換」「人間の傲慢さ」というテーマを原作で深く味わいたい人。",
    workId: "0zref9t",
  thumbnailUrl: "https://books.google.com/books/content?id=PQ7FDwAAQBAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "naruto-anime",
    mediaTitle: "NARUTO -ナルト-",
    mediaType: "anime",
    mediaYear: 2002,
    originalExists: true,
    originalTitle: "NARUTO",
    originalAuthor: "岸本斉史",
    originalType: "manga",
    adaptationLabel: "アニメ化",
    description:
      "忍者の里の落ちこぼれ少年・ナルトが火影を目指す忍者アクション。スタジオぴえろ制作。週刊少年ジャンプ連載。世界累計2.5億部超。",
    searchAliases: ["岸本斉史", "ナルト", "サスケ", "忍者", "忍里"],
    adaptationNotes:
      "アニメはオリジナルエピソード（アニオリ）が多数挿入されているため、原作漫画のほうがテンポよく読める。漫画全72巻。",
    recommendedFor:
      "アニメで忍者の熱い展開が好きだった人。成長・友情・裏切りのテーマを原作で追いたい人。",
    workId: "17otkk9",
  thumbnailUrl: "https://books.google.com/books/content?id=ji3NCwAAQBAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "bleach-anime",
    mediaTitle: "BLEACH",
    mediaType: "anime",
    mediaYear: 2004,
    originalExists: true,
    originalTitle: "BLEACH",
    originalAuthor: "久保帯人",
    originalType: "manga",
    adaptationLabel: "アニメ化",
    description:
      "死神代行として戦う高校生・一護の物語。スタジオぴえろ制作。週刊少年ジャンプ連載。2022年に「千年血戦篇」がアニメ化され大きな話題に。",
    searchAliases: ["久保帯人", "一護", "ルキア", "死神", "卍解"],
    adaptationNotes:
      "2022年の千年血戦篇アニメは高クオリティで旧来ファンを驚かせた。原作漫画全74巻は斬新なデザインと独自の世界観が特徴。",
    recommendedFor:
      "千年血戦篇アニメで感動した人。死神と虚の壮大な世界観を原作で読みたい人。",
    workId: "0xf2b4f",
  thumbnailUrl: "https://books.google.com/books/content?id=FQTwAQAAQBAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "pinpon-movie",
    mediaTitle: "ピンポン",
    mediaType: "movie",
    mediaYear: 2002,
    originalExists: true,
    originalTitle: "ピンポン",
    originalAuthor: "松本大洋",
    originalType: "manga",
    adaptationLabel: "映画化",
    description:
      "卓球の天才少年と凡才少年の青春を描く実写映画。窪塚洋介・ARATA主演。2014年にはアニメ版も制作された人気漫画が原作。",
    searchAliases: ["松本大洋", "ペコ", "スマイル", "卓球"],
    adaptationNotes:
      "映画は原作の雰囲気を見事に再現した名作として評価が高い。漫画全5巻は映画よりも内面描写と哲学的テーマが深く、短編として完成度が高い。",
    recommendedFor:
      "映画の疾走感と映像美が忘れられない人。才能・友情・アイデンティティをテーマにした漫画が好きな人。",
    workId: "1xytg7s",
  thumbnailUrl: "https://books.google.com/books/content?id=HQfzCgAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },

  // ── 2010年代前半 ─────────────────────────────────────────────────

  {
    id: "thermae-romae",
    mediaTitle: "テルマエ・ロマエ",
    mediaType: "movie",
    mediaYear: 2012,
    originalExists: true,
    originalTitle: "テルマエ・ロマエ",
    originalAuthor: "ヤマザキマリ",
    originalType: "manga",
    adaptationLabel: "映画化",
    description:
      "古代ローマの建築家が現代日本の風呂文化にタイムスリップするコメディ。阿部寛主演。興収59億円の大ヒット。月刊コミックビーム連載漫画が原作。",
    searchAliases: ["ヤマザキマリ", "ルシウス", "ローマ", "お風呂"],
    adaptationNotes:
      "映画は原作のユーモアをよく再現。漫画全6巻はローマ史の考証が丁寧で、映画よりもルシウスの葛藤と文化的考察が深い。",
    recommendedFor:
      "映画で笑いと感動を味わった人。ローマ史や浴場文化に興味がある人。",
    workId: "0up6xx8",
  thumbnailUrl: "https://books.google.com/books/content?id=BESSrIdvibYC&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "uchu-kyodai",
    mediaTitle: "宇宙兄弟",
    mediaType: "movie",
    mediaYear: 2012,
    originalExists: true,
    originalTitle: "宇宙兄弟",
    originalAuthor: "小山宙哉",
    originalType: "manga",
    adaptationLabel: "映画化",
    description:
      "幼い頃宇宙飛行士を夢見た兄弟の物語。小栗旬・岡田将生主演。同年アニメも放送。NASAやJAXAが監修した精密なリアリズムが特徴。",
    searchAliases: ["小山宙哉", "南波六太", "日々人", "宇宙飛行士", "NASA"],
    adaptationNotes:
      "映画は原作の序盤を1本に凝縮。漫画は長期連載中で宇宙開発の最前線と人間ドラマを丁寧に積み重ねており、映画よりはるかに深い。",
    recommendedFor:
      "映画で夢を追い続けることに勇気をもらった人。宇宙・科学好きが人間ドラマも楽しみたい場合。",
    workId: "1vgzl4a",
  thumbnailUrl: "https://books.google.com/books/content?id=KrtxEQAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "jojo-anime",
    mediaTitle: "ジョジョの奇妙な冒険",
    mediaType: "anime",
    mediaYear: 2012,
    originalExists: true,
    originalTitle: "ジョジョの奇妙な冒険",
    originalAuthor: "荒木飛呂彦",
    originalType: "manga",
    adaptationLabel: "アニメ化",
    description:
      "ジョースター家と宿敵ディオの因縁を描く壮大な冒険物語。david production制作。週刊少年ジャンプ→ウルトラジャンプ連載。全8部が映像化。",
    searchAliases: ["荒木飛呂彦", "ジョジョ", "ディオ", "スタンド", "波紋"],
    adaptationNotes:
      "アニメは各部ごとに高クオリティで映像化。原作漫画は部によって主人公・時代・世界観が大きく変わる独特の構成が魅力。",
    recommendedFor:
      "アニメで荒木ワールドのセンスに魅了された人。バトルの演出と独特のファッションセンスが好きな人。",
  },
  {
    id: "funa-o-amu",
    mediaTitle: "舟を編む",
    mediaType: "movie",
    mediaYear: 2013,
    originalExists: true,
    originalTitle: "舟を編む",
    originalAuthor: "三浦しをん",
    originalType: "novel",
    adaptationLabel: "映画化",
    description:
      "辞書編集者たちの15年にわたる奮闘を描く文学映画。松田龍平主演。本屋大賞受賞の原作小説。2013年には辞書作りの熱量が話題に。",
    searchAliases: ["三浦しをん", "辞書", "大渡海", "馬締"],
    adaptationNotes:
      "映画は原作の静かな情緒をよく再現。原作小説はより豊かな言葉の描写と、言葉に向き合う人々の内面が丁寧に書かれている。",
    recommendedFor:
      "映画で言葉・日本語・辞書というテーマに興味を持った人。静かで深い文学作品を読みたい人。",
    workId: "0zz63zd",
  thumbnailUrl: "https://books.google.com/books/content?id=EMUPrgEACAAJ&printsec=frontcover&img=1&zoom=1&imgtk=AFLRE70NT17-ktZP0AmqvWl7INBvUWur2tjnqQwsXdwvq5aUAUuIoqpZ9OqrpUs58ab9VJvsx9mCMMPUr84GSj-qgBj2HBeopVokExIWfXW_1f-7F3FGoR1a8yzASFiMPRhbyq9YQpF5&source=gbs_api",
  },
  {
    id: "eien-no-zero",
    mediaTitle: "永遠の0",
    mediaType: "movie",
    mediaYear: 2013,
    originalExists: true,
    originalTitle: "永遠の0",
    originalAuthor: "百田尚樹",
    originalType: "novel",
    adaptationLabel: "映画化",
    description:
      "特攻隊員だった祖父の真実を孫が追う戦争ドラマ。岡田准一主演。国内興収87億円超の大ヒット。本屋大賞受賞の累計700万部超の原作小説。",
    searchAliases: ["百田尚樹", "宮部久蔵", "特攻隊", "ゼロ戦"],
    adaptationNotes:
      "映画は原作の主要な場面を丁寧に映像化。原作小説はより複数の視点からの証言形式で、戦争の悲惨さと人間の葛藤がより深く伝わる。",
    recommendedFor:
      "映画で日本の戦争と家族愛のテーマに感動した人。歴史小説・戦争文学に入門したい人。",
    workId: "07j5tto",
  thumbnailUrl: "https://books.google.com/books/content?id=ofeFQgAACAAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "parasyte-anime",
    mediaTitle: "寄生獣 セイの格率",
    mediaType: "anime",
    mediaYear: 2014,
    originalExists: true,
    originalTitle: "寄生獣",
    originalAuthor: "岩明均",
    originalType: "manga",
    adaptationLabel: "アニメ化",
    description:
      "地球外生命体・パラサイトに侵食される世界で人間と共存する少年の物語。マッドハウス制作。1990年連載の傑作をフルリメイク。実写映画も同年公開。",
    searchAliases: ["岩明均", "新一", "ミギー", "パラサイト"],
    adaptationNotes:
      "アニメは原作漫画（全10巻）をほぼ忠実に映像化。漫画は白黒の荒削りな画風が独特の緊張感を生んでおり、アニメとは異なる質感が楽しめる。",
    recommendedFor:
      "アニメで「人間とは何か」というテーマに引き込まれた人。SFホラーとヒューマンドラマが好きな人。",
    workId: "1k8u22x",
  thumbnailUrl: "https://books.google.com/books/content?id=h4S_oQEACAAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "tokyo-ghoul-anime",
    mediaTitle: "東京喰種",
    mediaType: "anime",
    mediaYear: 2014,
    originalExists: true,
    originalTitle: "東京喰種トーキョーグール",
    originalAuthor: "石田スイ",
    originalType: "manga",
    adaptationLabel: "アニメ化",
    description:
      "人肉を食う「喰種」と人間の境界で生きる青年を描くダークファンタジー。スタジオぴえろ制作。週刊ヤングジャンプ連載。",
    searchAliases: ["石田スイ", "金木研", "カネキ", "グール"],
    adaptationNotes:
      "アニメは原作を大幅に改変・圧縮しており、特にルート（√A）以降は別の展開をたどる。原作漫画（全14巻）を読むとより整合性のある深い物語が理解できる。",
    recommendedFor:
      "アニメで世界観に引き込まれた人。原作でアニメとの違いを確かめたい人。人間とモンスターの境界をテーマにした作品が好きな人。",
    workId: "1szm3ez",
  thumbnailUrl: "https://books.google.com/books/content?id=-xrLCwAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "haikyuu-anime",
    mediaTitle: "ハイキュー!!",
    mediaType: "anime",
    mediaYear: 2014,
    originalExists: true,
    originalTitle: "ハイキュー!!",
    originalAuthor: "古舘春一",
    originalType: "manga",
    adaptationLabel: "アニメ化",
    description:
      "高校バレーを舞台に全国制覇を目指す少年たちの物語。Production I.G制作。週刊少年ジャンプ連載。国内外で絶大な人気を誇る。",
    searchAliases: ["古舘春一", "日向翔陽", "影山飛雄", "烏野高校", "バレーボール"],
    adaptationNotes:
      "アニメは原作に忠実で映像・音楽のクオリティが高く評価される。漫画は全45巻で完結しており、全国大会の決勝まで描かれている。2024年には劇場版も公開。",
    recommendedFor:
      "アニメで試合の熱さに感動した人。スポーツ漫画でチームワークと成長のテーマが好きな人。",
    workId: "1d7xco7",
  thumbnailUrl: "https://books.google.com/books/content?id=wI7LCwAAQBAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "shigatsu-wa-kimi-anime",
    mediaTitle: "四月は君の嘘",
    mediaType: "anime",
    mediaYear: 2014,
    originalExists: true,
    originalTitle: "四月は君の嘘",
    originalAuthor: "新川直司",
    originalType: "manga",
    adaptationLabel: "アニメ化",
    description:
      "ピアノの天才少年と自由奔放なバイオリニストの出会いと別れを描く青春音楽アニメ。A-1 Pictures制作。月刊少年マガジン連載。",
    searchAliases: ["新川直司", "有馬公生", "宮園かをり", "ピアノ", "バイオリン"],
    adaptationNotes:
      "アニメは原作全11巻をほぼ忠実に映像化。音楽の演奏シーンが特に高く評価される。漫画は音楽描写と心理描写が繊細で、涙なしには読めない。",
    recommendedFor:
      "アニメで音楽と感動の物語に引き込まれた人。青春・音楽・死と再生のテーマが好きな人。",
    workId: "0h9zmq9",
  thumbnailUrl: "https://books.google.com/books/content?id=n0fKBQAAQBAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "umi-machi-diary",
    mediaTitle: "海街diary",
    mediaType: "movie",
    mediaYear: 2015,
    originalExists: true,
    originalTitle: "海街diary",
    originalAuthor: "吉田秋生",
    originalType: "manga",
    adaptationLabel: "映画化",
    description:
      "鎌倉を舞台に4姉妹の日常を描く是枝裕和監督の人間ドラマ。綾瀬はるか・長澤まさみ・夏帆・広瀬すず主演。カンヌ国際映画祭コンペティション選出。",
    searchAliases: ["吉田秋生", "鎌倉", "4姉妹", "すず", "是枝裕和"],
    adaptationNotes:
      "映画は原作漫画の序盤をほぼ忠実に映像化。漫画は全9巻で、映画後の4姉妹のさらなる変化と鎌倉の四季が丁寧に描かれる。",
    recommendedFor:
      "映画で静かな感動を覚えた人。家族・姉妹・日常の美しさを描いた作品が好きな人。",
    workId: "01f86a7",
  thumbnailUrl: "https://books.google.com/books/content?id=QhSCDAAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "bakuman-movie",
    mediaTitle: "バクマン。",
    mediaType: "movie",
    mediaYear: 2015,
    originalExists: true,
    originalTitle: "バクマン。",
    originalAuthor: "大場つぐみ・小畑健",
    originalType: "manga",
    adaptationLabel: "映画化",
    description:
      "漫画家を目指す二人の少年の青春を描く実写映画。佐藤健・神木隆之介主演。「週刊少年ジャンプ」の世界を内側から描いた原作漫画。",
    searchAliases: ["大場つぐみ", "小畑健", "亜城木夢叶", "漫画家", "ジャンプ"],
    adaptationNotes:
      "映画はミュージカル的演出で原作を大胆に再構成。漫画全20巻はジャンプの仕組みや連載の過酷さをリアルに描いており、漫画好きに刺さる内容。",
    recommendedFor:
      "映画の熱い創作バトルに興奮した人。漫画家・創作の世界を深く知りたい人。",
    workId: "1hmxear",
  thumbnailUrl: "https://books.google.com/books/content?id=rhcljgEACAAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "ansatsu-kyoshitsu-movie",
    mediaTitle: "暗殺教室",
    mediaType: "movie",
    mediaYear: 2015,
    originalExists: true,
    originalTitle: "暗殺教室",
    originalAuthor: "松井優征",
    originalType: "manga",
    adaptationLabel: "映画化",
    description:
      "月を破壊した謎の生物を教師として「暗殺」する中学生たちの物語。山田涼介・二宮和也主演。週刊少年ジャンプ連載漫画の実写映画化。",
    searchAliases: ["松井優征", "殺せんせー", "渚", "業", "E組"],
    adaptationNotes:
      "映画は原作序盤のアークを中心に構成。漫画全21巻は各キャラクターの成長と終盤の感動的な結末まで描かれており、映画では味わえない深みがある。",
    recommendedFor:
      "映画で殺せんせーとE組の絆に感動した人。笑いと感動が共存する漫画を読みたい人。",
    workId: "05mqcs1",
  thumbnailUrl: "https://books.google.com/books/content?id=xZ_MCwAAQBAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "rurouni-kenshin-movie",
    mediaTitle: "るろうに剣心",
    mediaType: "movie",
    mediaYear: 2012,
    originalExists: true,
    originalTitle: "るろうに剣心",
    originalAuthor: "和月伸宏",
    originalType: "manga",
    adaptationLabel: "映画化",
    description:
      "明治時代の不殺の剣客・緋村剣心の物語。佐藤健主演。実写化困難とされた人気漫画の映像化に成功し、シリーズ5作で興収200億円超。",
    searchAliases: ["和月伸宏", "剣心", "緋村剣心", "薫", "志志雄"],
    adaptationNotes:
      "映画は原作の主要アーク（東京編・京都編・人誅編）をシリーズで映像化。漫画全28巻はキャラクターの背景や剣術の描写がより詳細。",
    recommendedFor:
      "映画のアクションと時代劇の世界観が好きな人。明治維新の歴史と剣の哲学を漫画で楽しみたい人。",
    workId: "0fcv3n3",
  thumbnailUrl: "https://books.google.com/books/content?id=JGPKCwAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "boku-no-hero-anime",
    mediaTitle: "僕のヒーローアカデミア",
    mediaType: "anime",
    mediaYear: 2016,
    originalExists: true,
    originalTitle: "僕のヒーローアカデミア",
    originalAuthor: "堀越耕平",
    originalType: "manga",
    adaptationLabel: "アニメ化",
    description:
      "個性（超能力）を持たない少年が最強ヒーローを目指すアクション。BONES制作。週刊少年ジャンプ連載。世界190カ国以上で配信される国際的ヒット作。",
    searchAliases: ["堀越耕平", "緑谷出久", "デク", "オールマイト", "ヒロアカ"],
    adaptationNotes:
      "アニメは原作に概ね忠実。漫画は完結しており、ヒーロー社会の崩壊と再建という壮大なエンディングまで描かれている（アニメはほぼ完結）。",
    recommendedFor:
      "アニメで「自分を超えていく」成長ドラマが好きな人。アメコミ風のヒーロー漫画が好きな人。",
    workId: "032sc3q",
  thumbnailUrl: "https://books.google.com/books/content?id=7iLLCwAAQBAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "konosuba-anime",
    mediaTitle: "この素晴らしい世界に祝福を！",
    mediaType: "anime",
    mediaYear: 2016,
    originalExists: true,
    originalTitle: "この素晴らしい世界に祝福を！",
    originalAuthor: "暁なつめ",
    originalType: "novel",
    adaptationLabel: "アニメ化",
    description:
      "異世界転生したダメ人間と個性豊かな仲間たちのコメディ。Studio DEEN制作。「なろう系」の元祖として知られるライトノベル原作。",
    searchAliases: ["暁なつめ", "カズマ", "アクア", "めぐみん", "ダクネス", "コノスバ"],
    adaptationNotes:
      "アニメはコメディのテンポが非常に良く、原作の笑いをうまく映像化。ライトノベル原作は17巻で完結しており、アニメよりさらに多くのエピソードが読める。",
    recommendedFor:
      "アニメでゆるいコメディに笑い転げた人。異世界転生ジャンルでコメディ寄りの作品が好きな人。",
    workId: "1r4zopm",
  thumbnailUrl: "https://books.google.com/books/content?id=59gNzwEACAAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },

  // ── 2010年代後半 ────────────────────────────────────────────────

  {
    id: "march-lion-movie",
    mediaTitle: "3月のライオン",
    mediaType: "movie",
    mediaYear: 2017,
    originalExists: true,
    originalTitle: "3月のライオン",
    originalAuthor: "羽海野チカ",
    originalType: "manga",
    adaptationLabel: "映画化",
    description:
      "孤独な天才将棋棋士・桐山零の成長と人との絆を描く実写映画。神木隆之介主演。前後編2作。月刊ヤングアニマル連載の人気漫画が原作。",
    searchAliases: ["羽海野チカ", "桐山零", "将棋", "川本", "三月のライオン"],
    adaptationNotes:
      "映画は原作の前半のエッセンスをうまく2本に凝縮。漫画は将棋の世界の奥深さとキャラクターの感情の細やかさが映画よりさらに豊か。",
    recommendedFor:
      "映画で孤独と再生のテーマに感動した人。将棋・家族・癒しをテーマにした漫画が好きな人。",
    workId: "0le0pd2",
  thumbnailUrl: "https://books.google.com/books/content?vid=ISBN9784592141105&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "okotoko",
    mediaTitle: "億男",
    mediaType: "movie",
    mediaYear: 2018,
    originalExists: true,
    originalTitle: "億男",
    originalAuthor: "川村元気",
    originalType: "novel",
    adaptationLabel: "映画化",
    description:
      "宝くじで3億円を当てた男が幸福とお金の意味を問う物語。佐藤健・高橋一生主演。「君の名は。」プロデューサー・川村元気の小説が原作。",
    searchAliases: ["川村元気", "お金", "幸福", "一男"],
    adaptationNotes:
      "映画は原作の構成をよく再現。小説は哲学的な問いをより深く掘り下げており、各章の仕掛けも読みどころ。",
    recommendedFor:
      "映画で「お金と幸せ」の問いに考えさせられた人。川村元気ファンや軽妙なエンタメ小説が好きな人。",
    workId: "1p3apoo",
  thumbnailUrl: "https://books.google.com/books/content?id=0265DAAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "tenki-danran-iken",
    mediaTitle: "転生したらスライムだった件",
    mediaType: "anime",
    mediaYear: 2018,
    originalExists: true,
    originalTitle: "転生したらスライムだった件",
    originalAuthor: "伏瀬",
    originalType: "novel",
    adaptationLabel: "アニメ化",
    description:
      "転生してスライムになった主人公がどんどん進化する異世界ファンタジー。8bit制作。「小説家になろう」発の人気ライトノベルシリーズ原作。",
    searchAliases: ["伏瀬", "リムル", "スライム", "テンスラ", "大賢者"],
    adaptationNotes:
      "アニメは原作のアニメ化に成功したが、原作小説（全25巻完結）のほうが世界観の設定と内面描写が圧倒的に詳細。",
    recommendedFor:
      "アニメで無双系異世界ファンタジーの爽快感が好きな人。国家経営系ストーリーが好きな人。",
    workId: "1aakpum",
  thumbnailUrl: "https://books.google.com/books/content?id=ZRrjEAAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "masquerade-hotel",
    mediaTitle: "マスカレード・ホテル",
    mediaType: "movie",
    mediaYear: 2019,
    originalExists: true,
    originalTitle: "マスカレード・ホテル",
    originalAuthor: "東野圭吾",
    originalType: "novel",
    adaptationLabel: "映画化",
    description:
      "ホテルに潜入捜査する刑事とフロントクラーク女性の連続殺人事件捜査。木村拓哉・長澤まさみ主演。東野圭吾の人気シリーズ小説が原作。",
    searchAliases: ["東野圭吾", "マスカレード", "新田", "山岸"],
    adaptationNotes:
      "映画は原作のホテルという密閉空間での謎解きをよく再現。原作は伏線の精密さとホテルの描写がより詳細。",
    recommendedFor:
      "映画で優雅な謎解きに引き込まれた人。東野圭吾ミステリーの優雅な世界観が好きな人。",
    workId: "069g6xa",
  thumbnailUrl: "https://books.google.com/books/content?id=Uo7toAEACAAJ&printsec=frontcover&img=1&zoom=1&imgtk=AFLRE72mZj0W2yBEDLCsICscw_WhimvO8k4J2UHltkkz6gwNpsfjSIz1Gxrjaf2hrMm7iLWAdb_86Xw_0E8uqhtWmrzJbgYmrSsxB24RpPBolVkf87kxVoGtWBiIbmaXckMPKRjLDItN&source=gbs_api",
  },
  {
    id: "mitsubachi-to-enrai",
    mediaTitle: "蜜蜂と遠雷",
    mediaType: "movie",
    mediaYear: 2019,
    originalExists: true,
    originalTitle: "蜜蜂と遠雷",
    originalAuthor: "恩田陸",
    originalType: "novel",
    adaptationLabel: "映画化",
    description:
      "国際ピアノコンクールを舞台に4人のピアニストの戦いと内面を描く音楽ドラマ。松岡茉優・松坂桃李主演。直木賞・本屋大賞W受賞の原作小説。",
    searchAliases: ["恩田陸", "コンクール", "栄伝亜夜", "高島明石", "マサル"],
    adaptationNotes:
      "映画は映像と音楽で原作の世界観を再現。原作小説（上下巻）はより濃密な心理描写と音楽描写で、各ピアニストの内面と演奏が言葉で丁寧に表現されている。",
    recommendedFor:
      "映画で音楽と才能のテーマに感動した人。クラシック音楽が好きな人。文学作品として深い読書体験をしたい人。",
    workId: "0mbk7f7",
  thumbnailUrl: "https://books.google.com/books/content?id=eGNVygEACAAJ&printsec=frontcover&img=1&zoom=1&imgtk=AFLRE71Xczj0OwjZZ7VEr0_Q5-Ty6BIbIAVVbQAbtbz8lu4HAJJR4MuWLJwYC67QZgqcMBDFZswn_C9ENEuR92VPx8i2DiKsouOfxup6jSwmN0tFCT1o3jlZxfdLsEX1twum64j7XMfT&source=gbs_api",
  },
  {
    id: "mushoku-tensei-anime",
    mediaTitle: "無職転生 〜異世界行ったら本気だす〜",
    mediaType: "anime",
    mediaYear: 2021,
    originalExists: true,
    originalTitle: "無職転生 〜異世界行ったら本気だす〜",
    originalAuthor: "理不尽な孫の手",
    originalType: "novel",
    adaptationLabel: "アニメ化",
    description:
      "人生をやり直した引きこもりが異世界で真剣に生きるファンタジー。スタジオバインド制作。「なろう系」ライトノベルの最高峰と称される作品。",
    searchAliases: ["理不尽な孫の手", "ルーデウス", "シルフィ", "無職転生"],
    adaptationNotes:
      "アニメの作画クオリティが高く評価されている。原作小説（全26巻完結）は心理描写と世界設定の緻密さが際立っており、アニメより深い没入感がある。",
    recommendedFor:
      "アニメで異世界ファンタジーの丁寧な世界観に惹かれた人。人生やり直し・成長テーマが好きな人。",
    workId: "00ge48i",
  thumbnailUrl: "https://books.google.com/books/content?id=4L8czgEACAAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "tsumi-no-koe",
    mediaTitle: "罪の声",
    mediaType: "movie",
    mediaYear: 2020,
    originalExists: true,
    originalTitle: "罪の声",
    originalAuthor: "塩田武士",
    originalType: "novel",
    adaptationLabel: "映画化",
    description:
      "昭和最大の未解決事件「グリコ・森永事件」を題材にした社会派ミステリー。小栗旬・星野源主演。著者・塩田武士による徹底取材を基にした小説。",
    searchAliases: ["塩田武士", "グリコ森永事件", "曽根俊也", "阿久津英士"],
    adaptationNotes:
      "映画は原作のエッセンスをよく凝縮。原作小説はジャーナリズムと家族の痛みを二軸で丁寧に描いており、映画より事件の詳細と登場人物の葛藤が深く描かれる。",
    recommendedFor:
      "映画で昭和の未解決事件ミステリーに引き込まれた人。実際の事件を基にした社会派小説が好きな人。",
    workId: "0ar8707",
  thumbnailUrl: "https://books.google.com/books/content?id=bI3mxQEACAAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "drive-my-car",
    mediaTitle: "ドライブ・マイ・カー",
    mediaType: "movie",
    mediaYear: 2021,
    originalExists: true,
    originalTitle: "ドライブ・マイ・カー",
    originalAuthor: "村上春樹",
    originalType: "novel",
    adaptationLabel: "映画化",
    description:
      "妻を失った舞台俳優と寡黙なドライバーの旅を描く濱口竜介監督作品。西島秀俊主演。アカデミー賞国際長編映画賞受賞。原作は短編小説集収録作品。",
    searchAliases: ["村上春樹", "濱口竜介", "家福悠介", "チェーホフ", "女のいない男たち"],
    adaptationNotes:
      "映画は約3時間の大作で原作の世界を大幅に拡張・映像化。原作は「女のいない男たち」に収録の短編で、映画とは異なる凝縮された文体で読める。",
    recommendedFor:
      "映画の静かで深い余韻が忘れられない人。村上春樹の原作短編と映画の差異を楽しみたい人。",
    workId: "11fitjc",
  thumbnailUrl: "https://books.google.com/books/content?id=gdDWoAEACAAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "danmachi-anime",
    mediaTitle: "ダンジョンに出会いを求めるのは間違っているだろうか",
    mediaType: "anime",
    mediaYear: 2015,
    originalExists: true,
    originalTitle: "ダンジョンに出会いを求めるのは間違っているだろうか",
    originalAuthor: "大森藤ノ",
    originalType: "novel",
    adaptationLabel: "アニメ化",
    description:
      "異世界の迷宮都市で冒険者として成長する少年の物語。J.C.STAFF制作。GA文庫のライトノベルシリーズが原作。「ダンまち」の略称で親しまれる。",
    searchAliases: ["大森藤ノ", "ベル", "ヘスティア", "ダンまち", "迷宮"],
    adaptationNotes:
      "アニメは各シーズンで原作の各巻に対応して映像化。ライトノベル原作（既刊20巻超）はアニメよりもキャラクターの内面と世界観の設定が詳細。",
    recommendedFor:
      "アニメで成長系ファンタジーの世界観にハマった人。ダンジョン探索と神様コミュニティという設定が好きな人。",
    workId: "0ecjsmn",
  thumbnailUrl: "https://books.google.com/books/content?id=JIbnnAAACAAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "yuru-camp-anime",
    mediaTitle: "ゆるキャン△",
    mediaType: "anime",
    mediaYear: 2018,
    originalExists: true,
    originalTitle: "ゆるキャン△",
    originalAuthor: "あfろ",
    originalType: "manga",
    adaptationLabel: "アニメ化",
    description:
      "女子高生たちのキャンプライフを描く癒し系アウトドア漫画のアニメ版。C-Station制作。月刊ComicFuz連載。キャンプブームに火をつけた作品。",
    searchAliases: ["あfろ", "リン", "なでしこ", "野外活動サークル", "キャンプ"],
    adaptationNotes:
      "アニメは原作の雰囲気を忠実に再現した高評価作。漫画はキャンプの実用情報も豊富で、アニメよりも各キャラクターの成長がゆっくり丁寧に描かれる。",
    recommendedFor:
      "アニメで癒しとキャンプの魅力を感じた人。アウトドア好き・ソロ活動好きの人に特にお薦め。",
    workId: "02ro5k9",
  thumbnailUrl: "https://books.google.com/books/content?id=gVfuEAAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },

  // ── 2020年代 ─────────────────────────────────────────────────────

  {
    id: "got-overseas",
    mediaTitle: "ゲーム・オブ・スローンズ",
    mediaType: "overseas",
    mediaYear: 2011,
    originalExists: true,
    originalTitle: "氷と炎の歌",
    originalAuthor: "ジョージ・R・R・マーティン",
    originalType: "novel",
    adaptationLabel: "HBOドラマ化（海外）",
    description:
      "王座を巡る貴族たちの血みどろの権力争いを描くファンタジー大作。HBOドラマ。シーズン8まで放送。世界中で大ヒットし現代ファンタジーの金字塔に。",
    searchAliases: ["ゲームオブスローンズ", "マーティン", "ウェスタロス", "ドラゴン", "ラニスター"],
    adaptationNotes:
      "ドラマはシーズン5まで概ね原作に沿い、シーズン6以降は未完の原作を先行して独自に展開。原作（5部作・未完）はより複雑な政治描写と視点が多い。",
    recommendedFor:
      "ドラマで壮大な権力ゲームの世界観に引き込まれた人。西洋ファンタジーの大作小説を原文・翻訳で読みたい人。",
    workId: "1m7e2ww",
  thumbnailUrl: "https://books.google.com/books/content?id=BI11QgAACAAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "sherlock-bbc",
    mediaTitle: "SHERLOCK（BBC）",
    mediaType: "overseas",
    mediaYear: 2010,
    originalExists: true,
    originalTitle: "シャーロック・ホームズ",
    originalAuthor: "アーサー・コナン・ドイル",
    originalType: "novel",
    adaptationLabel: "BBCドラマ化（海外）",
    description:
      "シャーロック・ホームズを現代ロンドンに置き換えたBBCドラマ。ベネディクト・カンバーバッチ主演。原作の名探偵を現代的に大胆リブート。",
    searchAliases: ["ホームズ", "ワトソン", "コナン・ドイル", "BBC", "カンバーバッチ"],
    adaptationNotes:
      "ドラマは原作の事件を現代版にアレンジしており、原作既読者には「元ネタ探し」も楽しめる。原作短編集は本格ミステリーの祖として現代でも色褪せない面白さ。",
    recommendedFor:
      "ドラマで頭脳明晰なホームズのキャラクターに惹かれた人。古典ミステリーを楽しみたい人。",
    workId: "0hnqg2i",
  thumbnailUrl: "https://books.google.com/books/content?id=6e3KAwAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },

  // ── 2015–2025 ───────────────────────────────────────────────────

  {
    id: "nigeru-wa-haji",
    mediaTitle: "逃げるは恥だが役に立つ",
    mediaType: "drama",
    mediaYear: 2016,
    originalExists: true,
    originalTitle: "逃げるは恥だが役に立つ",
    originalAuthor: "海野つなみ",
    originalType: "manga",
    adaptationLabel: "テレビドラマ化",
    description:
      "契約結婚から始まる恋愛コメディ。TBS系。星野源・新垣結衣主演。「恋ダンス」が社会現象に。Kiss連載漫画が原作。",
    searchAliases: ["にげはじ", "海野つなみ", "ガッキー", "星野源", "みくり"],
    adaptationNotes:
      "ドラマはほぼ原作に忠実だが、職場での男女格差・経済格差のテーマを現代的に強調している。原作漫画はドラマ後半の展開もさらに続く。",
    recommendedFor:
      "ドラマで「働き方」や「生き方」を考えさせられた人。ラブコメと社会派テーマを同時に楽しみたい人。",
    workId: "1foo4fe",
  thumbnailUrl: "https://books.google.com/books/content?id=V9c_DgAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "kounotori",
    mediaTitle: "コウノドリ",
    mediaType: "drama",
    mediaYear: 2015,
    originalExists: true,
    originalTitle: "コウノドリ",
    originalAuthor: "鈴ノ木ユウ",
    originalType: "manga",
    adaptationLabel: "テレビドラマ化",
    description:
      "産婦人科医が主人公の医療ドラマ。TBS系。綾野剛主演。出産・命・家族の葛藤をリアルに描く。週刊モーニング連載漫画が原作。",
    searchAliases: ["鈴ノ木ユウ", "産婦人科", "コウノドリ"],
    adaptationNotes:
      "ドラマは原作のエピソードをほぼ忠実に映像化。漫画は産科医療の倫理的問題をより深く掘り下げており、医療従事者にも高く評価されている。",
    recommendedFor:
      "命の誕生と医療の現実に関心がある人。感動的な医療ドラマを探している人。",
    workId: "1vrawnp",
  thumbnailUrl: "https://books.google.com/books/content?id=HIE-DwAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "rikuo-drama",
    mediaTitle: "陸王",
    mediaType: "drama",
    mediaYear: 2017,
    originalExists: true,
    originalTitle: "陸王",
    originalAuthor: "池井戸潤",
    originalType: "novel",
    adaptationLabel: "テレビドラマ化",
    description:
      "足袋製造の老舗中小企業がランニングシューズ開発に挑む物語。TBS系。役所広司主演。直木賞作家・池井戸潤の原作小説。",
    searchAliases: ["池井戸潤", "ランニングシューズ", "陸王"],
    adaptationNotes:
      "ドラマは原作のストーリーラインを忠実に追いながらも、キャラクターの描写を若干変更。ものづくりと組織の葛藤は原作でより深く描かれる。",
    recommendedFor:
      "ドラマで熱い展開に感動した人。中小企業の奮闘を描く経済小説が好きな人。",
    workId: "1ycw3o9",
  thumbnailUrl: "https://books.google.com/books/content?id=GWLvxgEACAAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "mystery-to-iu-nakare",
    mediaTitle: "ミステリと言う勿れ",
    mediaType: "drama",
    mediaYear: 2022,
    originalExists: true,
    originalTitle: "ミステリと言う勿れ",
    originalAuthor: "田村由美",
    originalType: "manga",
    adaptationLabel: "テレビドラマ化",
    description:
      "独特の語り口と鋭い観察眼を持つ青年・久能整が事件に巻き込まれるミステリー。フジテレビ系。菅田将暉主演。",
    searchAliases: ["田村由美", "久能整", "ミステリ言う勿れ"],
    adaptationNotes:
      "ドラマは原作の代表的なエピソードをよく再現。漫画は既刊20巻超で、ドラマ化されていない事件やキャラクターの深掘りが続く。",
    recommendedFor:
      "ドラマで久能整のセリフに共感した人。社会問題・哲学的テーマを絡めたミステリーが好きな人。",
    workId: "0436z1i",
  thumbnailUrl: "https://books.google.com/books/content?id=RpiiswEACAAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "kingdom-movie",
    mediaTitle: "キングダム",
    mediaType: "movie",
    mediaYear: 2019,
    originalExists: true,
    originalTitle: "キングダム",
    originalAuthor: "原泰久",
    originalType: "manga",
    adaptationLabel: "映画化",
    description:
      "中国春秋戦国時代を舞台にした歴史漫画の実写映画版。山崎賢人・吉沢亮主演。シリーズ累計1億部超の人気漫画。",
    searchAliases: ["原泰久", "信", "嬴政", "春秋戦国"],
    adaptationNotes:
      "映画は原作序盤のアーク（山の民）を圧縮して映像化。漫画はより壮大なスケールで長期連載が続いており、映画よりはるかに多くの戦略・人物が描かれる。",
    recommendedFor:
      "映画でスケールの大きさに引かれた人。歴史・戦略漫画の大作を読みたい人。",
    workId: "0u6ruh5",
  thumbnailUrl: "https://books.google.com/books/content?id=bCXKCwAAQBAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "tonde-saitama",
    mediaTitle: "翔んで埼玉",
    mediaType: "movie",
    mediaYear: 2019,
    originalExists: true,
    originalTitle: "翔んで埼玉",
    originalAuthor: "魔夜峰央",
    originalType: "manga",
    adaptationLabel: "映画化",
    description:
      "埼玉県民への壮大なディスりギャグ漫画の実写映画版。GACKT・二階堂ふみ主演。興収37億円超の大ヒット。",
    searchAliases: ["魔夜峰央", "さいたま", "翔んで埼玉"],
    adaptationNotes:
      "映画は原作の設定・世界観を膨らませてオリジナルストーリーを加えた作品。原作漫画は1982年連載の短編で、映画のほうがボリュームが多い。",
    recommendedFor:
      "映画で爆笑した人。ギャグ・パロディ漫画の元祖を読んでみたい人。",
    workId: "07f06ti",
  thumbnailUrl: "https://books.google.com/books/content?id=wVOVjwEACAAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "tokyo-revengers-movie",
    mediaTitle: "東京リベンジャーズ",
    mediaType: "movie",
    mediaYear: 2021,
    originalExists: true,
    originalTitle: "東京卍リベンジャーズ",
    originalAuthor: "和久井健",
    originalType: "manga",
    adaptationLabel: "映画化",
    description:
      "タイムリープで過去に戻り悪の組織を変えようとする青春ヤンキーアクション。北村匠海主演。週刊少年マガジン連載。",
    searchAliases: ["和久井健", "東リベ", "マイキー", "タケミチ"],
    adaptationNotes:
      "映画は原作序盤の東京卍會との戦いを中心に構成。原作漫画（全31巻）はタイムリープと複雑な組織の因果関係がより詳細に描かれる。",
    recommendedFor:
      "映画でタイムリープと友情の物語にハマった人。ヤンキー漫画×サスペンスが好きな人。",
    workId: "0sz79up",
  thumbnailUrl: "https://books.google.com/books/content?id=A5_XtAEACAAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "tenki-no-ko",
    mediaTitle: "天気の子",
    mediaType: "movie",
    mediaYear: 2019,
    originalExists: true,
    originalTitle: "天気の子",
    originalAuthor: "新海誠",
    originalType: "novel",
    adaptationLabel: "映画→ノベライズ",
    description:
      "家出少年と「晴れ女」の少女の出会いを描く新海誠監督作品。国内興収142億円超。映画公開と同時にノベライズが刊行。",
    searchAliases: ["新海誠", "天気の子", "ほだか", "ひな"],
    adaptationNotes:
      "小説は映画と同内容だが、帆高・陽菜それぞれの視点から語られる心理描写が丁寧に補完されている。映画後に読むと世界観がより深まる。",
    recommendedFor:
      "映画の余韻に浸りたい人。君の名は。を楽しんだ人。新海誠の世界観を活字で体験したい人。",
    workId: "1ytttd6",
  thumbnailUrl: "https://books.google.com/books/content?id=6SkvyQEACAAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "slam-dunk-movie",
    mediaTitle: "THE FIRST SLAM DUNK",
    mediaType: "movie",
    mediaYear: 2022,
    originalExists: true,
    originalTitle: "SLAM DUNK",
    originalAuthor: "井上雄彦",
    originalType: "manga",
    adaptationLabel: "映画化",
    description:
      "伝説のバスケ漫画を原作者・井上雄彦自身が監督した劇場版。国内興収159億円超の大ヒット。週刊少年ジャンプの名作が原作。",
    searchAliases: ["スラムダンク", "井上雄彦", "桜木花道", "流川楓"],
    adaptationNotes:
      "映画はリョータを主人公に山王戦を描くオリジナル視点の構成。原作漫画（全31巻）は桜木を主人公に湘北の成長を全体通して描く。どちらから入っても楽しめる。",
    recommendedFor:
      "映画で感動した人・ドリブルのシーンが忘れられない人。青春バスケ漫画の金字塔を読みたい人。",
    workId: "15ther4",
  thumbnailUrl: "https://books.google.com/books/content?id=aRhXygEACAAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "suzume",
    mediaTitle: "すずめの戸締まり",
    mediaType: "movie",
    mediaYear: 2022,
    originalExists: true,
    originalTitle: "すずめの戸締まり",
    originalAuthor: "新海誠",
    originalType: "novel",
    adaptationLabel: "映画→ノベライズ",
    description:
      "日本各地の「廃墟の扉」を閉めて回る少女の旅を描く新海誠監督作品。国内興収149億円超。映画公開と同時にノベライズが刊行。",
    searchAliases: ["新海誠", "すずめの戸締まり", "宗像草太"],
    adaptationNotes:
      "小説では映画で語られない登場人物の背景・内面が丁寧に補完されている。災害・記憶・復興のテーマが活字でより深く感じられる。",
    recommendedFor:
      "映画の余韻を大切にしたい人。東日本大震災と記憶というテーマに向き合いたい人。",
    workId: "053jq1a",
  thumbnailUrl: "https://books.google.com/books/content?id=1QgN0AEACAAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "blue-giant-movie",
    mediaTitle: "BLUE GIANT",
    mediaType: "movie",
    mediaYear: 2023,
    originalExists: true,
    originalTitle: "BLUE GIANT",
    originalAuthor: "石塚真一",
    originalType: "manga",
    adaptationLabel: "映画化",
    description:
      "東北出身の青年がジャズサックスで世界を目指す青春音楽漫画の映画版。ビッグコミック連載。音楽制作に上原ひろみが参加。",
    searchAliases: ["石塚真一", "ジャズ", "サックス", "大"],
    adaptationNotes:
      "映画は原作の「東京編」を中心に再構成。漫画シリーズはヨーロッパ・アメリカ編へと続いており、ジャズを軸にした成長の物語がさらに広がる。",
    recommendedFor:
      "映画の演奏シーンに心を動かされた人。音楽・ジャズに興味を持った人。青春と挑戦の物語が好きな人。",
    workId: "1n6ojyj",
  thumbnailUrl: "https://books.google.com/books/content?id=ia_6CgAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "jujutsu-kaisen-anime",
    mediaTitle: "呪術廻戦",
    mediaType: "anime",
    mediaYear: 2020,
    originalExists: true,
    originalTitle: "呪術廻戦",
    originalAuthor: "芥見下々",
    originalType: "manga",
    adaptationLabel: "アニメ化",
    description:
      "呪霊と戦う呪術師たちを描くダークファンタジーアクション。MAPPA制作。週刊少年ジャンプ連載。劇場版・第2期も大ヒット。",
    searchAliases: ["じゅじゅつかいせん", "芥見下々", "虎杖悠仁", "五条悟"],
    adaptationNotes:
      "アニメは原作に忠実で、映像・音楽のクオリティが特に高く評価される。漫画は全26巻で完結。渋谷事変以降の展開がアニメ第2期・3期で映像化されている。",
    recommendedFor:
      "アニメで世界観に引き込まれた人。バトルと哲学的テーマが融合した漫画が好きな人。",
    workId: "1er2ue6",
  thumbnailUrl: "https://books.google.com/books/content?id=XhvH0QEACAAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "spy-family-anime",
    mediaTitle: "SPY×FAMILY",
    mediaType: "anime",
    mediaYear: 2022,
    originalExists: true,
    originalTitle: "SPY×FAMILY",
    originalAuthor: "遠藤達哉",
    originalType: "manga",
    adaptationLabel: "アニメ化",
    description:
      "スパイ・殺し屋・超能力者の三人が偽家族を演じるコメディ。WIT STUDIO×CloverWorks制作。少年ジャンプ＋連載。",
    searchAliases: ["スパイファミリー", "遠藤達哉", "ロイド", "アーニャ", "ヨル"],
    adaptationNotes:
      "アニメは原作に忠実で、アーニャのリアクション演技が特に人気。漫画は最新話まで連載中でアニメ未収録の展開が続く。",
    recommendedFor:
      "アニメで笑いと感動を味わった人。スパイアクション×家族コメディという組み合わせが好きな人。",
    workId: "1r7geub",
  thumbnailUrl: "https://books.google.com/books/content?id=0dSZDwAAQBAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "chainsaw-man-anime",
    mediaTitle: "チェンソーマン",
    mediaType: "anime",
    mediaYear: 2022,
    originalExists: true,
    originalTitle: "チェンソーマン",
    originalAuthor: "藤本タツキ",
    originalType: "manga",
    adaptationLabel: "アニメ化",
    description:
      "チェンソーの悪魔と合体した少年・デンジが悪魔狩りをするダークアクション。MAPPA制作。週刊少年ジャンプ連載。",
    searchAliases: ["藤本タツキ", "デンジ", "パワー", "マキマ"],
    adaptationNotes:
      "アニメはPART1（公安編）を映像化。映像・音楽の演出が高く評価されるが、漫画PART2（学校編）はまだアニメ化されておらず原作でしか読めない。",
    recommendedFor:
      "アニメの独特な演出と世界観にハマった人。不条理・バイオレンス系の漫画が好きな人。",
    workId: "04hn7u2",
  thumbnailUrl: "https://books.google.com/books/publisher/content?id=NyOEDwAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&imgtk=AFLRE73HQcUg7k2hy6mBM4ztXIPmJP450vFE73Mw21xeOJIQG_dEenybCegvoDyhoO5qDc6T1O8REj88_3p6oRmuPkDoW1JEoSR77sUjT9I6jdXxCXxJYF7v8xe3zVzSohnGtWSB-WJT&source=gbs_api",
  },
  {
    id: "blue-lock-anime",
    mediaTitle: "ブルーロック",
    mediaType: "anime",
    mediaYear: 2022,
    originalExists: true,
    originalTitle: "ブルーロック",
    originalAuthor: "金城宗幸",
    originalType: "manga",
    adaptationLabel: "アニメ化",
    description:
      "日本最高のストライカーを育てる極限サバイバルサッカー漫画のアニメ版。週刊少年マガジン連載。シリーズ累計3000万部超。",
    searchAliases: ["金城宗幸", "ブルーロック", "潔世一", "サッカー"],
    adaptationNotes:
      "アニメは原作の前半（ブルーロック施設編）を映像化。漫画は欧州リーグ編へと展開が続いており、アニメよりさらに先の物語が読める。",
    recommendedFor:
      "アニメで極限の競争と才能の物語にハマった人。スポーツ漫画で心理戦・個人主義テーマが好きな人。",
    workId: "156varz",
  thumbnailUrl: "https://books.google.com/books/content?id=5Ft2DwAAQBAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "frieren-anime",
    mediaTitle: "葬送のフリーレン",
    mediaType: "anime",
    mediaYear: 2023,
    originalExists: true,
    originalTitle: "葬送のフリーレン",
    originalAuthor: "山田鐘人",
    originalType: "manga",
    adaptationLabel: "アニメ化",
    description:
      "魔王討伐後の世界を旅するエルフの魔法使いを描くファンタジー。Madhouse制作。週刊少年サンデー連載。アニメ賞多数受賞。",
    searchAliases: ["山田鐘人", "フリーレン", "ヒンメル", "エルフ"],
    adaptationNotes:
      "アニメは原作に非常に忠実で、作画・音楽のクオリティが特に高い。漫画は連載継続中でアニメ第2期以降の展開が原作で先行している。",
    recommendedFor:
      "アニメで「時間・別れ・人の営み」というテーマに心打たれた人。ファンタジーを哲学的視点で楽しみたい人。",
    workId: "154nqi5",
  thumbnailUrl: "https://books.google.com/books/content?id=jJnjzQEACAAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "kusuriya-anime",
    mediaTitle: "薬屋のひとりごと",
    mediaType: "anime",
    mediaYear: 2023,
    originalExists: true,
    originalTitle: "薬屋のひとりごと",
    originalAuthor: "日向夏",
    originalType: "novel",
    adaptationLabel: "アニメ化",
    description:
      "後宮を舞台に薬師の少女・猫猫が謎を解くミステリー。OLM/TOHO Animation制作。小説・漫画（2種類の漫画版）が原作。",
    searchAliases: ["日向夏", "猫猫", "壬氏", "後宮"],
    adaptationNotes:
      "アニメは漫画版（ビッグガンガン連載）を主に参考に映像化。ライトノベル原作はより文章的な情報密度が高く、後宮の政治背景が詳細。",
    recommendedFor:
      "アニメで後宮ミステリーの世界観に引き込まれた人。中華風ファンタジーと謎解きが好きな人。",
    workId: "0g6vt24",
  thumbnailUrl: "https://books.google.com/books/content?id=7C_GDwAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
  },
  {
    id: "kaiju-8-anime",
    mediaTitle: "怪獣8号",
    mediaType: "anime",
    mediaYear: 2024,
    originalExists: true,
    originalTitle: "怪獣8号",
    originalAuthor: "松本直也",
    originalType: "manga",
    adaptationLabel: "アニメ化",
    description:
      "怪獣に変身できる青年が防衛隊員を目指すアクション漫画のアニメ版。Production I.G制作。少年ジャンプ＋連載。",
    searchAliases: ["松本直也", "怪獣8号", "日比野化", "防衛隊"],
    adaptationNotes:
      "アニメは原作の前半を忠実に映像化。漫画は連載継続中でアニメ未収録の展開が読める。",
    recommendedFor:
      "アニメで怪獣×人間ドラマの組み合わせにハマった人。努力と成長のバトル漫画が好きな人。",
    workId: "08tvese",
  thumbnailUrl: "https://books.google.com/books/content?id=jRgFEAAAQBAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "one-piece-netflix",
    mediaTitle: "ONE PIECE（Netflix実写版）",
    mediaType: "overseas",
    mediaYear: 2023,
    originalExists: true,
    originalTitle: "ONE PIECE",
    originalAuthor: "尾田栄一郎",
    originalType: "manga",
    adaptationLabel: "Netflix実写化",
    description:
      "世界累計5億部超の海賊冒険漫画のNetflixオリジナル実写ドラマ。東バラジ・マガスハキ主演。シーズン1は東の海編を映像化。",
    searchAliases: ["ワンピース", "尾田栄一郎", "ルフィ", "麦わらの一味"],
    adaptationNotes:
      "実写版は原作の大ファンである尾田栄一郎が制作に深く関与しており、原作へのリスペクトが高く評価されている。原作漫画は1000話超の大作。",
    recommendedFor:
      "実写版から原作に興味を持った人。冒険・仲間・意志のテーマが好きな人。",
    workId: "0cb8t8s",
  thumbnailUrl: "https://books.google.com/books/content?id=9ca7CwAAQBAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "santi-netflix",
    mediaTitle: "三体",
    mediaType: "overseas",
    mediaYear: 2024,
    originalExists: true,
    originalTitle: "三体",
    originalAuthor: "劉慈欣",
    originalType: "novel",
    adaptationLabel: "Netflix実写化",
    description:
      "中国SFの金字塔をNetflixが実写ドラマ化。文化大革命から始まる壮大なファーストコンタクト物語。ヒューゴー賞受賞の原作小説。",
    searchAliases: ["リュウジキン", "さんたい", "ファーストコンタクト", "SF"],
    adaptationNotes:
      "Netflix版は舞台を現代のイギリスに変更しているが、物語の核心は忠実に再現。小説の三部作（三体・黒暗森林・死神永生）は膨大なスケールのSF叙事詩。",
    recommendedFor:
      "ドラマでスケールの大きさに圧倒された人。ハードSFの最高峰に挑戦したい人。",
    workId: "0w8wdw1",
  thumbnailUrl: "https://books.google.com/books/content?id=BZV9zgEACAAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },
  {
    id: "blue-period-anime",
    mediaTitle: "ブルーピリオド",
    mediaType: "anime",
    mediaYear: 2021,
    originalExists: true,
    originalTitle: "ブルーピリオド",
    originalAuthor: "山口つばさ",
    originalType: "manga",
    adaptationLabel: "アニメ化",
    description:
      "成績優秀なリア充が美術に目覚め東京藝大を目指す青春漫画のアニメ版。月刊アフタヌーン連載。アニメーション制作：七緒。",
    searchAliases: ["山口つばさ", "矢口八虎", "東京藝大", "美術"],
    adaptationNotes:
      "アニメは原作の藝大受験まで（序盤）を映像化。漫画は入学後の苦悩・成長・アイデンティティの模索が続いており、アニメよりはるかに長い。",
    recommendedFor:
      "アニメで「なぜ絵を描くのか」という問いに共感した人。芸術・創作・自己表現をテーマにした漫画が好きな人。",
    workId: "0295sk0",
  thumbnailUrl: "https://books.google.com/books/content?id=W8qGswEACAAJ&printsec=frontcover&img=1&zoom=1&source=gbs_api",
  },

  // === 追加分（Part1: 50件） ===

  // --- movie (映画) ---
  {
    id: "kingdom-manga-movie",
    mediaTitle: "キングダム",
    mediaType: "movie",
    mediaYear: 2019,
    originalExists: true,
    originalTitle: "キングダム",
    originalAuthor: "原泰久",
    originalType: "manga",
    adaptationLabel: "漫画原作",
    description: "中国の春秋戦国時代を舞台に、奴隷の少年・信が天下の大将軍を目指す壮大な歴史漫画の実写映画化。",
    searchAliases: ["キングダム", "kingdom", "原泰久"],
    adaptationNotes: "実写映画化でシリーズ化。壮大な合戦シーンが話題に。漫画は70巻超の大作で、秦の中国統一を描く歴史大河。",
    recommendedFor: "歴史好き・大河ドラマ好きに。映画を見た後に漫画で詳細なストーリーを追いたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "rurouni-kenshin-manga-movie",
    mediaTitle: "るろうに剣心",
    mediaType: "movie",
    mediaYear: 2012,
    originalExists: true,
    originalTitle: "るろうに剣心 明治剣客浪漫譚",
    originalAuthor: "和月伸宏",
    originalType: "manga",
    adaptationLabel: "漫画原作",
    description: "明治時代を舞台に、不殺の誓いを立てた元人斬り・緋村剣心の活躍を描いた伝説的剣劇漫画の実写映画。",
    searchAliases: ["るろうに剣心", "rurouni kenshin", "和月伸宏", "剣心"],
    adaptationNotes: "佐藤健主演。シリーズ5作が製作され大ヒット。原作漫画は90年代の名作で現在も愛読者が多い。",
    recommendedFor: "アクション映画ファンや明治時代に興味がある方に。シリーズを見た後に原作漫画で補完したい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "kimi-no-suizo-wo-tabetai-novel-movie",
    mediaTitle: "君の膵臓をたべたい",
    mediaType: "movie",
    mediaYear: 2017,
    originalExists: true,
    originalTitle: "君の膵臓をたべたい",
    originalAuthor: "住野よる",
    originalType: "novel",
    adaptationLabel: "小説原作",
    description: "余命わずかな少女と内向的な男子高校生の、秘密を共有した特別な日々を描く青春恋愛小説の映画化。",
    searchAliases: ["君の膵臓をたべたい", "きみすい", "住野よる"],
    adaptationNotes: "浜辺美波・北村匠海主演の実写版と2018年のアニメ映画版がある。原作小説はデビュー作にして大ベストセラー。",
    recommendedFor: "泣ける恋愛映画が好きな方に。映画の感動をより深く味わいたい方が原作小説を読むのにおすすめ。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "kagami-no-kojou-novel-movie",
    mediaTitle: "かがみの孤城",
    mediaType: "movie",
    mediaYear: 2022,
    originalExists: true,
    originalTitle: "かがみの孤城",
    originalAuthor: "辻村深月",
    originalType: "novel",
    adaptationLabel: "小説原作",
    description: "学校に居場所をなくした7人の中学生が鏡の中の城に集められ、謎解きをしながら絆を深めるアニメ映画。",
    searchAliases: ["かがみの孤城", "辻村深月", "鏡の孤城"],
    adaptationNotes: "原恵一監督のアニメ映画。本屋大賞受賞作の映像化。切なくも温かいラストは原作・映画ともに高評価。",
    recommendedFor: "不登校・居場所をテーマにした物語に共感できる方に。アニメ映画を見て原作の細部まで読みたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "mystery-to-iu-nakare-manga-movie",
    mediaTitle: "ミステリと言う勿れ",
    mediaType: "movie",
    mediaYear: 2023,
    originalExists: true,
    originalTitle: "ミステリと言う勿れ",
    originalAuthor: "田村由美",
    originalType: "manga",
    adaptationLabel: "漫画原作",
    description: "独自の思考と膨大な知識を持つ大学生・久能整が様々な事件に巻き込まれる異色ミステリ漫画の映画化。",
    searchAliases: ["ミステリと言う勿れ", "ミステリといわなかれ", "田村由美", "久能整"],
    adaptationNotes: "菅田将暉主演でドラマ化・映画化。整くんの長台詞が話題に。原作漫画はFlowers連載の人気作。",
    recommendedFor: "ミステリ好きだが一般的な謎解きと違う視点を求める方に。独特のキャラクターに魅了された方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "sen-wa-boku-wo-egaku-novel-movie",
    mediaTitle: "線は、僕を描く",
    mediaType: "movie",
    mediaYear: 2022,
    originalExists: true,
    originalTitle: "線は、僕を描く",
    originalAuthor: "砥上裕將",
    originalType: "novel",
    adaptationLabel: "小説原作",
    description: "両親を事故で亡くした大学生が水墨画の巨匠に弟子入りし、喪失と向き合いながら成長する青春小説の映画化。",
    searchAliases: ["線は僕を描く", "砥上裕將", "水墨画"],
    adaptationNotes: "横浜流星主演。本屋大賞受賞作の映像化。水墨画の世界が美しく映像化され、原作の繊細な心理描写が話題に。",
    recommendedFor: "喪失と再生をテーマにした物語が好きな方に。美術・芸術に興味がある方や静寂な作品が好きな方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "yomei-junen-novel-movie",
    mediaTitle: "余命10年",
    mediaType: "movie",
    mediaYear: 2022,
    originalExists: true,
    originalTitle: "余命10年",
    originalAuthor: "小坂流加",
    originalType: "novel",
    adaptationLabel: "小説原作",
    description: "難病で余命10年を宣告された20代の女性が、恋をしないと決意しながらも青年に惹かれていく純愛小説の映画化。",
    searchAliases: ["余命10年", "小坂流加", "余命"],
    adaptationNotes: "坂口健太郎・小松菜奈主演。著者自身の実体験を基にした半自伝的小説。著者は2017年に逝去。",
    recommendedFor: "切ない純愛物語が好きな方に。限られた時間を精一杯生きることを描いた感動作を求める方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "rurosha-no-tsuki-novel-movie",
    mediaTitle: "流浪の月",
    mediaType: "movie",
    mediaYear: 2022,
    originalExists: true,
    originalTitle: "流浪の月",
    originalAuthor: "凪良ゆう",
    originalType: "novel",
    adaptationLabel: "小説原作",
    description: "誘拐犯と被害者という関係で出会った二人が大人になって再会し、歪んだ真実が明らかになる社会派恋愛映画。",
    searchAliases: ["流浪の月", "凪良ゆう", "るろうのつき"],
    adaptationNotes: "広瀬すず・松坂桃李主演、李相日監督。本屋大賞受賞作。社会の偏見と固定観念に挑む問題作。",
    recommendedFor: "社会問題を扱った重厚な恋愛作品が好きな方に。本屋大賞作品を映像で楽しんだ後に原作も読みたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "seiyoku-novel-movie",
    mediaTitle: "正欲",
    mediaType: "movie",
    mediaYear: 2023,
    originalExists: true,
    originalTitle: "正欲",
    originalAuthor: "朝井リョウ",
    originalType: "novel",
    adaptationLabel: "小説原作",
    description: "「多様性」という言葉の裏に隠された生きづらさを、複数の視点から描く朝井リョウの社会派小説の映画化。",
    searchAliases: ["正欲", "朝井リョウ", "せいよく"],
    adaptationNotes: "稲垣吾郎・新垣結衣・磯村勇斗主演。朝井リョウの問題提起が映像でも鋭く表現されている問題作。",
    recommendedFor: "多様性や社会問題について深く考えたい方に。映画を見て朝井リョウの鋭い筆致を原作で味わいたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "golden-kamuy-manga-movie",
    mediaTitle: "ゴールデンカムイ",
    mediaType: "movie",
    mediaYear: 2024,
    originalExists: true,
    originalTitle: "ゴールデンカムイ",
    originalAuthor: "野田サトル",
    originalType: "manga",
    adaptationLabel: "漫画原作",
    description: "明治時代の北海道を舞台に、隠された金塊を巡り元兵士とアイヌ少女が繰り広げる冒険漫画の実写映画化。",
    searchAliases: ["ゴールデンカムイ", "golden kamuy", "野田サトル", "アイヌ"],
    adaptationNotes: "山崎賢人主演。アイヌ文化・北海道の自然を丁寧に再現。週刊ヤングジャンプ連載の人気漫画が原作。",
    recommendedFor: "歴史・サバイバル・アクションが好きな方に。アイヌ文化に興味がある方や北海道の自然が好きな方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "heroine-shikkaku-manga-movie",
    mediaTitle: "ヒロイン失格",
    mediaType: "movie",
    mediaYear: 2015,
    originalExists: true,
    originalTitle: "ヒロイン失格",
    originalAuthor: "幸田もも子",
    originalType: "manga",
    adaptationLabel: "漫画原作",
    description: "幼馴染への恋を叶えようと奮闘するヒロイン・はつが、恋愛の主人公になれない自分に気づく青春ラブコメの映画化。",
    searchAliases: ["ヒロイン失格", "幸田もも子", "はつ"],
    adaptationNotes: "橋本環奈・山崎賢人主演。少女漫画原作の学園恋愛映画。コミカルな演技とリアルな恋愛模様が話題に。",
    recommendedFor: "少女漫画系の恋愛映画が好きな方に。ヒロインの不器用さと成長を描いた物語を楽しみたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "koe-no-katachi-manga-movie",
    mediaTitle: "聲の形",
    mediaType: "movie",
    mediaYear: 2016,
    originalExists: true,
    originalTitle: "聲の形",
    originalAuthor: "大今良時",
    originalType: "manga",
    adaptationLabel: "漫画原作",
    description: "かつてイジメを行った少年と、聴覚障害を持つ少女の葛藤と赦しを描く京都アニメーション制作のアニメ映画。",
    searchAliases: ["聲の形", "こえのかたち", "大今良時", "A Silent Voice"],
    adaptationNotes: "京都アニメーション制作。山田尚子監督。週マガ連載原作。繊細な感情表現と美しい映像が世界的に高評価。",
    recommendedFor: "いじめ・罪と赦しをテーマにした作品に関心がある方に。アニメ映画の感動を原作漫画でも追体験したい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "vivant-novel-drama",
    mediaTitle: "VIVANT",
    mediaType: "drama",
    mediaYear: 2023,
    originalExists: true,
    originalTitle: "VIVANT",
    originalAuthor: "池井戸潤",
    originalType: "novel",
    adaptationLabel: "小説原作",
    description: "誤送金事件から始まりモンゴルの秘密組織へと発展する、TBS日曜劇場の大型スパイエンターテイメントドラマ。",
    searchAliases: ["VIVANT", "ヴィヴァン", "池井戸潤", "別班"],
    adaptationNotes: "堺雅人・阿部寛主演の超大作ドラマ。モンゴルロケが話題に。ノベライズ版が放送後に発売。",
    recommendedFor: "スパイ・サスペンスが好きな方に。ドラマを見た後に池井戸潤の筆致で読み直したい方に。",
    workId: "",
    thumbnailUrl: "",
  },

  // --- drama (日本のドラマ) ---
  {
    id: "watashi-no-shiawase-na-kekkon-drama",
    mediaTitle: "わたしの幸せな結婚",
    mediaType: "drama",
    mediaYear: 2023,
    originalExists: true,
    originalTitle: "わたしの幸せな結婚",
    originalAuthor: "顎木あくみ",
    originalType: "novel",
    adaptationLabel: "小説原作",
    description: "異能の力が当然の近代日本を舞台に、冷酷と噂の名家の長男に嫁いだ不遇な少女の幸せを描くラブファンタジードラマ。",
    searchAliases: ["わたしの幸せな結婚", "わたしこん", "顎木あくみ"],
    adaptationNotes: "目黒蓮・今田美桜主演。小説はラノベ発のベストセラー。ドラマと映画の両方が制作された人気シリーズ。",
    recommendedFor: "和風ファンタジーと切ない恋愛が好きな方に。ドラマを見て原作ライトノベルの世界をより深く楽しみたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "brushup-life-novel-drama",
    mediaTitle: "ブラッシュアップライフ",
    mediaType: "drama",
    mediaYear: 2023,
    originalExists: true,
    originalTitle: "ブラッシュアップライフ",
    originalAuthor: "バカリズム",
    originalType: "novel",
    adaptationLabel: "脚本・ノベライズ",
    description: "33歳で死んだ女性がやり直しの人生でも同じ友人たちと関わり続ける時間軸ループコメディドラマのノベライズ。",
    searchAliases: ["ブラッシュアップライフ", "バカリズム", "安藤なつ"],
    adaptationNotes: "バカリズム脚本のオリジナルドラマ。安藤なつ主演。ノベライズ版はドラマ放送後に発売。伏線回収が絶品。",
    recommendedFor: "タイムループもので笑って泣きたい方に。ドラマを見た後に本で改めて細部の伏線を確認したい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "jimenshatachi-novel-drama",
    mediaTitle: "地面師たち",
    mediaType: "drama",
    mediaYear: 2024,
    originalExists: true,
    originalTitle: "地面師たち",
    originalAuthor: "新庄耕",
    originalType: "novel",
    adaptationLabel: "小説原作",
    description: "土地取引詐欺師集団「地面師」の実態を描いた衝撃のクライム小説をNetflixが実写ドラマ化した問題作。",
    searchAliases: ["地面師たち", "新庄耕", "地面師", "Netflix"],
    adaptationNotes: "豊川悦司・綾野剛主演のNetflixドラマ。実際の積水ハウス詐欺事件をモデルに。衝撃の展開が話題を呼んだ。",
    recommendedFor: "クライム・サスペンス好きに。詐欺師の心理と手口を描いた社会派作品に興味がある方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "hanzawa-naoki-novel-drama",
    mediaTitle: "半沢直樹",
    mediaType: "drama",
    mediaYear: 2013,
    originalExists: true,
    originalTitle: "オレたちバブル入行組",
    originalAuthor: "池井戸潤",
    originalType: "novel",
    adaptationLabel: "小説原作",
    description: "理不尽な上司に倍返しで立ち向かう銀行員・半沢直樹の痛快なサラリーマン出世劇を描いた大ヒットドラマ。",
    searchAliases: ["半沢直樹", "池井戸潤", "倍返し", "オレたちバブル入行組"],
    adaptationNotes: "堺雅人主演。視聴率42.2%を記録した伝説的ドラマ。池井戸潤の「半沢直樹シリーズ」が原作。",
    recommendedFor: "サラリーマンの痛快な逆転劇が好きな方に。職場での理不尽に共感・スカッとしたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "shinya-shokudo-manga-drama",
    mediaTitle: "深夜食堂",
    mediaType: "drama",
    mediaYear: 2009,
    originalExists: true,
    originalTitle: "深夜食堂",
    originalAuthor: "安倍夜郎",
    originalType: "manga",
    adaptationLabel: "漫画原作",
    description: "深夜0時から朝5時まで営業する新宿の小料理屋を舞台に、客たちの人生模様を描く大人の人情ドラマ。",
    searchAliases: ["深夜食堂", "安倍夜郎", "めしや"],
    adaptationNotes: "小林薫主演。ドラマ・映画・Netflixシリーズと長寿コンテンツに。ビッグコミックオリジナル連載の原作漫画も人気。",
    recommendedFor: "人情ものや大人が楽しめるしんみりした作品が好きな方に。食と人生をテーマにした短編集を楽しみたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "nodame-cantabile-manga-drama",
    mediaTitle: "のだめカンタービレ（ドラマ）",
    mediaType: "drama",
    mediaYear: 2006,
    originalExists: true,
    originalTitle: "のだめカンタービレ",
    originalAuthor: "二ノ宮知子",
    originalType: "manga",
    adaptationLabel: "漫画原作",
    description: "天才的だが破天荒なピアノ学生・のだめと、厳格なエリート音楽学生・千秋の成長と恋愛を描く音楽コメディドラマ。",
    searchAliases: ["のだめカンタービレ", "二ノ宮知子", "のだめ", "千秋"],
    adaptationNotes: "上野樹里・玉木宏主演。クラシック音楽への入口として話題に。原作漫画はKissで連載された大人気作。",
    recommendedFor: "クラシック音楽に興味を持ちたい方に。笑いと音楽と恋愛が詰まった作品を楽しみたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "kimi-wa-petto-manga-drama",
    mediaTitle: "きみはペット",
    mediaType: "drama",
    mediaYear: 2003,
    originalExists: true,
    originalTitle: "きみはペット",
    originalAuthor: "小川彌生",
    originalType: "manga",
    adaptationLabel: "漫画原作",
    description: "キャリアウーマンの女性が偶然拾った青年を「ペット」として同居させるという型破りな恋愛漫画のドラマ化。",
    searchAliases: ["きみはペット", "小川彌生", "君はペット"],
    adaptationNotes: "松嶋菜々子・松本潤主演版と2012年の韓国ドラマ版がある。少女漫画ながら大人の恋愛観を描いた名作。",
    recommendedFor: "独特の恋愛関係を描いた少女漫画が好きな方に。女性が主体的な恋愛作品を楽しみたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "yoru-kanransha-novel-drama",
    mediaTitle: "夜行観覧車",
    mediaType: "drama",
    mediaYear: 2013,
    originalExists: true,
    originalTitle: "夜行観覧車",
    originalAuthor: "湊かなえ",
    originalType: "novel",
    adaptationLabel: "小説原作",
    description: "高級住宅街で起きた一家殺傷事件を、隣人家族の視点から描く湊かなえの家族サスペンス小説のドラマ化。",
    searchAliases: ["夜行観覧車", "湊かなえ", "よるのかんらんしゃ"],
    adaptationNotes: "鈴木京香・原田泰造主演。湊かなえ特有のどんでん返しと家族の闇を映像で再現。イヤミスの女王の代表作。",
    recommendedFor: "家族のリアルな闇と嘘を描いたサスペンスが好きな方に。湊かなえファンやイヤミス好きの方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "gin-no-saji-manga-drama",
    mediaTitle: "銀の匙 Silver Spoon",
    mediaType: "drama",
    mediaYear: 2014,
    originalExists: true,
    originalTitle: "銀の匙 Silver Spoon",
    originalAuthor: "荒川弘",
    originalType: "manga",
    adaptationLabel: "漫画原作",
    description: "都会育ちの少年が農業高校に入学し、農業・畜産・食の現実を学びながら成長する青春漫画の実写ドラマ化。",
    searchAliases: ["銀の匙", "Silver Spoon", "荒川弘", "銀スプ"],
    adaptationNotes: "中島健人主演の映画版も制作。農業リアリズムとコメディが融合した荒川弘の名作。食と命の大切さを描く。",
    recommendedFor: "農業・食・命をテーマにした作品に興味がある方に。笑いながら考えさせられる青春漫画を読みたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "moteki-manga-drama",
    mediaTitle: "モテキ",
    mediaType: "drama",
    mediaYear: 2010,
    originalExists: true,
    originalTitle: "モテキ",
    originalAuthor: "久保ミツロウ",
    originalType: "manga",
    adaptationLabel: "漫画原作",
    description: "非モテの29歳男子が突然複数の女性にモテ始める、音楽と恋愛が交差するリアルな青春恋愛コメディドラマ。",
    searchAliases: ["モテキ", "久保ミツロウ", "藤本幸世"],
    adaptationNotes: "森山未來主演のドラマと2011年の映画版。90〜00年代の音楽が多数使用。非モテ男子のリアルな心理が刺さる。",
    recommendedFor: "恋愛に不器用な主人公の成長を描いた作品が好きな方に。音楽と恋愛が交差する作品を楽しみたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "silent-novel-drama",
    mediaTitle: "silent",
    mediaType: "drama",
    mediaYear: 2022,
    originalExists: true,
    originalTitle: "silent",
    originalAuthor: "生方美久",
    originalType: "novel",
    adaptationLabel: "脚本・ノベライズ",
    description: "10年ぶりに再会した初恋の人が聴覚を失っていた。手話と愛と過去をめぐる切ない恋愛ドラマのノベライズ版。",
    searchAliases: ["silent", "サイレント", "生方美久", "手話"],
    adaptationNotes: "川口春奈・目黒蓮主演のフジテレビドラマ。ノベライズ版は脚本家・生方美久が手掛けた。月9の名作として語り継がれる。",
    recommendedFor: "切ない純愛ドラマが好きな方に。手話・聴覚障害をテーマにした恋愛作品に関心がある方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "mahoutsukai-no-yome-manga-drama",
    mediaTitle: "魔法使いの嫁 〜星待つひと〜",
    mediaType: "drama",
    mediaYear: 2024,
    originalExists: true,
    originalTitle: "魔法使いの嫁",
    originalAuthor: "ヤマザキコレ",
    originalType: "manga",
    adaptationLabel: "漫画原作",
    description: "ファンタジー漫画「魔法使いの嫁」を実写ドラマ化。魔法使いに買われた少女・チセが英国の魔法世界で成長する物語。",
    searchAliases: ["魔法使いの嫁", "まほよめ", "ヤマザキコレ", "チセ", "エリアス"],
    adaptationNotes: "月刊サンデーS連載の人気漫画をNHKが実写ドラマ化。アニメ版とは異なる実写ならではの表現が話題に。",
    recommendedFor: "英国ファンタジーが好きな方に。孤独な少女の成長と不思議な世界を描いた作品を楽しみたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "osen-manga-drama",
    mediaTitle: "おせん",
    mediaType: "drama",
    mediaYear: 2008,
    originalExists: true,
    originalTitle: "おせん",
    originalAuthor: "きくち正太",
    originalType: "manga",
    adaptationLabel: "漫画原作",
    description: "老舗料理屋を切り盛りする和服美人・おせんが職人の誇りと日本の食文化を守る姿を描くドラマ。",
    searchAliases: ["おせん", "きくち正太", "和食", "料理屋"],
    adaptationNotes: "蒼井優主演。イブニング連載の原作漫画を実写ドラマ化。日本の伝統食文化と職人気質をテーマにした大人向け作品。",
    recommendedFor: "日本の食文化・職人仕事に関心がある方に。しっとりとした大人の物語を楽しみたい方に。",
    workId: "",
    thumbnailUrl: "",
  },

  // --- anime (アニメ) ---
  {
    id: "oshi-no-ko-manga-anime",
    mediaTitle: "【推しの子】",
    mediaType: "anime",
    mediaYear: 2023,
    originalExists: true,
    originalTitle: "【推しの子】",
    originalAuthor: "赤坂アカ・横槍メンゴ",
    originalType: "manga",
    adaptationLabel: "漫画原作",
    description: "人気アイドルに転生した双子の兄妹が芸能界の闇を暴く、衝撃の展開が続くサスペンス×芸能ドラマアニメ。",
    searchAliases: ["推しの子", "oshi no ko", "赤坂アカ", "横槍メンゴ", "YOASOBI"],
    adaptationNotes: "主題歌「アイドル」がYOASOBIにより世界的ヒット。週ヤン連載の人気漫画。芸能界の裏側を描く異色作。",
    recommendedFor: "芸能・アイドル業界の裏側に興味がある方に。転生ものだが恋愛コメディより本格ドラマ寄りの作品を求める方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "dungeon-meshi-manga-anime",
    mediaTitle: "ダンジョン飯",
    mediaType: "anime",
    mediaYear: 2024,
    originalExists: true,
    originalTitle: "ダンジョン飯",
    originalAuthor: "九井諒子",
    originalType: "manga",
    adaptationLabel: "漫画原作",
    description: "ダンジョンの魔物を食材として料理しながら冒険するファンタジー×グルメ漫画をスタジオトリガーがアニメ化。",
    searchAliases: ["ダンジョン飯", "dungeon meshi", "九井諒子", "delicious in dungeon"],
    adaptationNotes: "TRIGGER制作。2024年アニメで高評価。料理描写の丁寧さとキャラクターの魅力が原作・アニメともに絶賛される。",
    recommendedFor: "ファンタジー×食のユニークな組み合わせが好きな方に。世界観の作り込みが深い作品を楽しみたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "tokyo-revengers-manga-anime",
    mediaTitle: "東京リベンジャーズ",
    mediaType: "anime",
    mediaYear: 2021,
    originalExists: true,
    originalTitle: "東京卍リベンジャーズ",
    originalAuthor: "和久井健",
    originalType: "manga",
    adaptationLabel: "漫画原作",
    description: "死んだ元カノを救うためタイムリープを繰り返し、不良グループの抗争に飛び込む青春タイムリープ漫画のアニメ。",
    searchAliases: ["東京リベンジャーズ", "東卍", "和久井健", "tokyo revengers"],
    adaptationNotes: "週マガ連載。アニメと実写映画が制作。タイムリープ×不良×青春という独自の組み合わせが人気を博した。",
    recommendedFor: "タイムリープと不良漫画の融合に興味がある方に。過去を変えることで大切な人を守る物語を読みたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "kusuriya-no-hitorigoto-novel-anime",
    mediaTitle: "薬屋のひとりごと",
    mediaType: "anime",
    mediaYear: 2023,
    originalExists: true,
    originalTitle: "薬屋のひとりごと",
    originalAuthor: "日向夏",
    originalType: "novel",
    adaptationLabel: "小説原作",
    description: "後宮に売られた薬師の少女・猫猫が毒と薬の知識を活かして宮廷の謎を解く中華風ミステリファンタジーアニメ。",
    searchAliases: ["薬屋のひとりごと", "くすりやのひとりごと", "猫猫", "日向夏"],
    adaptationNotes: "小説・漫画（2種）・アニメと展開。猫猫の知的キャラと壬氏との関係性が人気。2023〜2024年の話題作。",
    recommendedFor: "中華風ファンタジーと謎解きが好きな方に。賢くて個性的な女性主人公の物語を楽しみたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "kaijuu-8-go-manga-anime",
    mediaTitle: "怪獣8号",
    mediaType: "anime",
    mediaYear: 2024,
    originalExists: true,
    originalTitle: "怪獣8号",
    originalAuthor: "松本直也",
    originalType: "manga",
    adaptationLabel: "漫画原作",
    description: "怪獣が日常的に現れる世界で、怪獣を除去する仕事に就く32歳の男が怪獣になる能力を得る少年漫画のアニメ。",
    searchAliases: ["怪獣8号", "カイジュウ8号", "松本直也", "kafka"],
    adaptationNotes: "少年ジャンプ＋連載。Netflixでも配信。主人公の年齢設定の珍しさと爽快なバトルが人気。2024年春アニメで話題。",
    recommendedFor: "怪獣もの・特撮好きに。遅咲きの主人公が本気になるアツい展開が好きな方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "dr-stone-manga-anime",
    mediaTitle: "Dr.STONE",
    mediaType: "anime",
    mediaYear: 2019,
    originalExists: true,
    originalTitle: "Dr.STONE",
    originalAuthor: "稲垣理一郎・Boichi",
    originalType: "manga",
    adaptationLabel: "漫画原作",
    description: "突然石化した人類が3700年後の石器時代で目覚め、科学の力で文明を再建する知識×冒険の少年漫画のアニメ。",
    searchAliases: ["Dr.STONE", "ドクターストーン", "稲垣理一郎", "Boichi", "千空"],
    adaptationNotes: "週ジャン連載。科学の知識が実際に正確と評判で教育的側面も。石器時代から文明を作る逆転発想が面白い。",
    recommendedFor: "理科・化学に興味がある方に。知識と頭脳で困難を乗り越えるキャラクターの物語を楽しみたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "vinland-saga-manga-anime",
    mediaTitle: "ヴィンランド・サガ",
    mediaType: "anime",
    mediaYear: 2019,
    originalExists: true,
    originalTitle: "ヴィンランド・サガ",
    originalAuthor: "幸村誠",
    originalType: "manga",
    adaptationLabel: "漫画原作",
    description: "11世紀のヴァイキング時代を舞台に、父の仇を追う少年戦士が真の戦士の道を歩む壮大な歴史漫画のアニメ。",
    searchAliases: ["ヴィンランドサガ", "vinland saga", "幸村誠", "トルフィン"],
    adaptationNotes: "WIT STUDIO→MAPPAと制作会社が変わりながらも高クオリティを維持。アフタヌーン連載の長編歴史漫画。",
    recommendedFor: "北欧・ヴァイキング歴史に興味がある方に。戦いと平和を深く問う骨太な物語を楽しみたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "sono-bisque-doll-manga-anime",
    mediaTitle: "その着せ替え人形は恋をする",
    mediaType: "anime",
    mediaYear: 2022,
    originalExists: true,
    originalTitle: "その着せ替え人形は恋をする",
    originalAuthor: "福田晋一",
    originalType: "manga",
    adaptationLabel: "漫画原作",
    description: "雛人形作りを志す職人気質の高校生と陽気なコスプレ好き女子高生がコスプレ制作を通じて距離を縮める恋愛漫画のアニメ。",
    searchAliases: ["その着せ替え人形は恋をする", "着せ恋", "福田晋一", "マリン"],
    adaptationNotes: "CloverWorks制作。コスプレとオタク文化への愛情が溢れる作品。ヒロインのマリンが強烈なキャラとして人気を博した。",
    recommendedFor: "コスプレ・オタク文化に関心がある方に。不器用な主人公と積極的なヒロインの恋愛を楽しみたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "bocchi-the-rock-manga-anime",
    mediaTitle: "ぼっち・ざ・ろっく！",
    mediaType: "anime",
    mediaYear: 2022,
    originalExists: true,
    originalTitle: "ぼっち・ざ・ろっく！",
    originalAuthor: "はまじあき",
    originalType: "manga",
    adaptationLabel: "漫画原作",
    description: "ぼっちな陰キャ少女がバンドに誘われ音楽と友人を通じて成長するロック漫画を、CloverWorksが高品質アニメ化。",
    searchAliases: ["ぼっち・ざ・ろっく", "ぼざろ", "はまじあき", "bocchi the rock"],
    adaptationNotes: "2022年秋アニメで大ヒット。アニメのクオリティが原作を超えたとの声も。バンド音楽の演奏シーンが圧巻。",
    recommendedFor: "バンド・音楽が好きな方に。人見知りの主人公の成長と友情を描いた青春作品を楽しみたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "mahou-tsukai-no-yome-manga-anime",
    mediaTitle: "魔法使いの嫁",
    mediaType: "anime",
    mediaYear: 2017,
    originalExists: true,
    originalTitle: "魔法使いの嫁",
    originalAuthor: "ヤマザキコレ",
    originalType: "manga",
    adaptationLabel: "漫画原作",
    description: "身寄りのない少女が魔法使いに買い取られ、魔法の世界で弟子兼花嫁として成長するファンタジー漫画のアニメ。",
    searchAliases: ["魔法使いの嫁", "まほよめ", "ヤマザキコレ", "古書見習い"],
    adaptationNotes: "WIT STUDIO制作。英国を舞台にした幻想的な世界観が美麗な映像で再現。月刊サンデーS連載の人気作。",
    recommendedFor: "英国ファンタジー・魔法世界が好きな方に。少女の成長と師弟を超えた愛を描いた物語を楽しみたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "kaguya-sama-manga-anime",
    mediaTitle: "かぐや様は告らせたい",
    mediaType: "anime",
    mediaYear: 2019,
    originalExists: true,
    originalTitle: "かぐや様は告らせたい～天才たちの恋愛頭脳戦～",
    originalAuthor: "赤坂アカ",
    originalType: "manga",
    adaptationLabel: "漫画原作",
    description: "名門高校の生徒会長と副会長が、互いに告白させようと心理戦を繰り広げるラブコメ漫画のアニメ化。",
    searchAliases: ["かぐや様は告らせたい", "かぐや様", "赤坂アカ", "白銀御行", "藤原千花"],
    adaptationNotes: "A-1 Pictures制作でシリーズ3期＋映画化。原作漫画は週ヤン連載で2023年完結。テンポの良いギャグと純愛が融合。",
    recommendedFor: "テンポの良いラブコメが好きな方に。心理戦と笑いと恋愛が詰まった作品を楽しみたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "re-zero-novel-anime",
    mediaTitle: "Re:ゼロから始める異世界生活",
    mediaType: "anime",
    mediaYear: 2016,
    originalExists: true,
    originalTitle: "Re:ゼロから始める異世界生活",
    originalAuthor: "長月達平",
    originalType: "novel",
    adaptationLabel: "小説原作",
    description: "異世界に召喚され「死に戻り」の能力を持つ青年が死を繰り返しながら仲間を守る異世界ファンタジーアニメ。",
    searchAliases: ["Re:ゼロ", "リゼロ", "長月達平", "スバル", "エミリア", "レム"],
    adaptationNotes: "白鯨・魔女の書etc、死に戻りを使った伏線構造が緻密。原作ライトノベルはまだ連載中で深い心理描写が魅力。",
    recommendedFor: "ダーク系異世界ファンタジーが好きな方に。精神的苦痛を乗り越えて成長する主人公の物語を読みたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "overlord-novel-anime",
    mediaTitle: "オーバーロード",
    mediaType: "anime",
    mediaYear: 2015,
    originalExists: true,
    originalTitle: "オーバーロード",
    originalAuthor: "丸山くがね",
    originalType: "novel",
    adaptationLabel: "小説原作",
    description: "終了したゲームの世界に取り残されたプレイヤーが、最強のアンデッド魔法使いとして異世界を征服するアニメ。",
    searchAliases: ["オーバーロード", "overlord", "丸山くがね", "アインズ", "ナザリック"],
    adaptationNotes: "Madhouse制作でシリーズ4期まで制作。原作ライトノベルは18巻で完結。悪の側から世界を描く逆転の発想が人気。",
    recommendedFor: "最強主人公系の異世界ファンタジーが好きな方に。ゲーム世界観を活かした緻密な設定の物語を楽しみたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "tensei-slime-novel-anime",
    mediaTitle: "転生したらスライムだった件",
    mediaType: "anime",
    mediaYear: 2018,
    originalExists: true,
    originalTitle: "転生したらスライムだった件",
    originalAuthor: "伏瀬",
    originalType: "novel",
    adaptationLabel: "小説原作",
    description: "サラリーマンがスライムに転生し、様々な種族と仲間になりながら国家を建設する異世界ファンタジーアニメ。",
    searchAliases: ["転生したらスライムだった件", "転スラ", "伏瀬", "リムル"],
    adaptationNotes: "8bitスタジオ制作でシリーズ3期まで制作。原作はWeb小説発のライトノベルで劇場版も制作された人気作。",
    recommendedFor: "チートな主人公が国造りをするほのぼの系異世界ファンタジーが好きな方に。",
    workId: "",
    thumbnailUrl: "",
  },

  // --- overseas (海外映画・ドラマ) ---
  {
    id: "game-of-thrones-novel-overseas",
    mediaTitle: "ゲーム・オブ・スローンズ",
    mediaType: "overseas",
    mediaYear: 2011,
    originalExists: true,
    originalTitle: "氷と炎の歌",
    originalAuthor: "ジョージ・R・R・マーティン",
    originalType: "novel",
    adaptationLabel: "小説原作",
    description: "7つの王国の王座を巡る貴族たちの謀略と戦争を描くHBOの超大作ファンタジードラマ。全8シーズン放映。",
    searchAliases: ["ゲーム・オブ・スローンズ", "GOT", "ジョージRRマーティン", "氷と炎の歌", "A Song of Ice and Fire"],
    adaptationNotes: "原作「氷と炎の歌」シリーズは全5巻（未完）。ドラマはシーズン8で原作を超えた独自展開をとった。",
    recommendedFor: "政治的陰謀と壮大なファンタジーが好きな方に。ドラマを見た後に原作小説の詳細な世界観を楽しみたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "harry-potter-novel-overseas",
    mediaTitle: "ハリー・ポッター",
    mediaType: "overseas",
    mediaYear: 2001,
    originalExists: true,
    originalTitle: "ハリー・ポッターと賢者の石",
    originalAuthor: "J・K・ローリング",
    originalType: "novel",
    adaptationLabel: "小説原作",
    description: "魔法使いの少年ハリーが魔法学校で友情・冒険・悪と戦いを経験する世界的ベストセラーファンタジーの映画化。",
    searchAliases: ["ハリー・ポッター", "Harry Potter", "J.K.ローリング", "ホグワーツ"],
    adaptationNotes: "全8作の映画シリーズ。世界で5億部超のベストセラー原作。映画と原作それぞれに異なる細部の描写がある。",
    recommendedFor: "ファンタジー好きの子どもから大人まで幅広くおすすめ。映画から入って原作7巻を完全に読みたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "dune-novel-overseas",
    mediaTitle: "DUNE/デューン 砂の惑星",
    mediaType: "overseas",
    mediaYear: 2021,
    originalExists: true,
    originalTitle: "デューン 砂の惑星",
    originalAuthor: "フランク・ハーバート",
    originalType: "novel",
    adaptationLabel: "小説原作",
    description: "砂漠の惑星アラキスを舞台に、選ばれし若者が覚醒する壮大なSF叙事詩をドゥニ・ヴィルヌーヴが映像化。",
    searchAliases: ["DUNE", "デューン", "フランク・ハーバート", "砂の惑星", "アラキス"],
    adaptationNotes: "ドゥニ・ヴィルヌーヴ監督の映画は2部作。原作は1965年のSF古典。スターウォーズなど多くの作品に影響を与えた。",
    recommendedFor: "スケールの大きなSF叙事詩が好きな方に。映画の世界観に魅了され原作小説の深い政治・宗教描写を読みたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "witcher-novel-overseas",
    mediaTitle: "ウィッチャー",
    mediaType: "overseas",
    mediaYear: 2019,
    originalExists: true,
    originalTitle: "ウィッチャー",
    originalAuthor: "アンジェイ・サプコフスキ",
    originalType: "novel",
    adaptationLabel: "小説原作",
    description: "怪物退治の専門家ウィッチャーが複雑な世界で依頼をこなすNetflixのダークファンタジードラマシリーズ。",
    searchAliases: ["ウィッチャー", "The Witcher", "アンジェイ・サプコフスキ", "ゲラルト"],
    adaptationNotes: "ヘンリー・カヴィル主演。ポーランドの小説が原作で、人気ゲーム「ウィッチャー3」の元でもある作品。",
    recommendedFor: "ダークファンタジーとゲームが好きな方に。複雑な世界観と皮肉的な主人公の物語を楽しみたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "lord-of-the-rings-novel-overseas",
    mediaTitle: "ロード・オブ・ザ・リング",
    mediaType: "overseas",
    mediaYear: 2001,
    originalExists: true,
    originalTitle: "指輪物語",
    originalAuthor: "J・R・R・トールキン",
    originalType: "novel",
    adaptationLabel: "小説原作",
    description: "指輪の力に立ち向かう9人の仲間の旅を描くファンタジー映画の金字塔。ピーター・ジャクソン監督の3部作。",
    searchAliases: ["ロード・オブ・ザ・リング", "指輪物語", "トールキン", "LOTR", "フロド"],
    adaptationNotes: "アカデミー賞多数受賞の大作映画。原作「指輪物語」はファンタジー文学の原点。翻訳は瀬田貞二・田中明子版が定番。",
    recommendedFor: "ファンタジー映画の原点を楽しみたい方に。映画を見て原作のより詳細な世界観・歴史・言語を知りたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "sherlock-bbc-novel-overseas",
    mediaTitle: "SHERLOCK（BBC）",
    mediaType: "overseas",
    mediaYear: 2010,
    originalExists: true,
    originalTitle: "シャーロック・ホームズ",
    originalAuthor: "アーサー・コナン・ドイル",
    originalType: "novel",
    adaptationLabel: "小説原作",
    description: "シャーロック・ホームズを現代ロンドンに置き換え、スマートフォンを使う天才探偵を描くBBCドラマシリーズ。",
    searchAliases: ["SHERLOCK", "シャーロック", "コナン・ドイル", "ホームズ", "BBC"],
    adaptationNotes: "ベネディクト・カンバーバッチ主演。原作はドイル全60編の短編・長編シリーズ。現代版解釈が秀逸。",
    recommendedFor: "ミステリ・推理小説好きに。古典ホームズを読んだことがある方が現代版との違いを楽しみたい場合にも。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "handmaids-tale-novel-overseas",
    mediaTitle: "ハンドメイズ・テイル/侍女の物語",
    mediaType: "overseas",
    mediaYear: 2017,
    originalExists: true,
    originalTitle: "侍女の物語",
    originalAuthor: "マーガレット・アトウッド",
    originalType: "novel",
    adaptationLabel: "小説原作",
    description: "女性の権利が剥奪された近未来国家で子どもを産む機械として扱われる侍女の恐怖と抵抗を描くHuluドラマ。",
    searchAliases: ["ハンドメイズ・テイル", "侍女の物語", "マーガレット・アトウッド", "The Handmaid's Tale"],
    adaptationNotes: "エミー賞多数受賞。原作は1985年発表のディストピア小説の傑作。続編「誓願」も原作・ドラマ両方で制作。",
    recommendedFor: "ディストピア小説・フェミニズム文学に関心がある方に。社会制度への批評を含む重厚な作品を楽しみたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "gone-girl-novel-overseas",
    mediaTitle: "ゴーン・ガール",
    mediaType: "overseas",
    mediaYear: 2014,
    originalExists: true,
    originalTitle: "ゴーン・ガール",
    originalAuthor: "ギリアン・フリン",
    originalType: "novel",
    adaptationLabel: "小説原作",
    description: "結婚記念日に妻が失踪し夫が容疑者となるが、真実は予想外の場所にあったデビッド・フィンチャー監督のサスペンス映画。",
    searchAliases: ["ゴーン・ガール", "Gone Girl", "ギリアン・フリン", "フィンチャー"],
    adaptationNotes: "ロザムンド・パイク・ベン・アフレック主演。原作小説は二重語りの構成が巧妙。ギリアン・フリン自身が脚本担当。",
    recommendedFor: "どんでん返しサスペンスが好きな方に。結婚・メディア・社会の歪みを描いたダークな作品を楽しみたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "da-vinci-code-novel-overseas",
    mediaTitle: "ダ・ヴィンチ・コード",
    mediaType: "overseas",
    mediaYear: 2006,
    originalExists: true,
    originalTitle: "ダ・ヴィンチ・コード",
    originalAuthor: "ダン・ブラウン",
    originalType: "novel",
    adaptationLabel: "小説原作",
    description: "ルーブル美術館での殺人事件から始まるキリスト教と西洋美術の謎を解くロバート・ラングドン教授の冒険映画。",
    searchAliases: ["ダ・ヴィンチ・コード", "The Da Vinci Code", "ダン・ブラウン", "ラングドン"],
    adaptationNotes: "トム・ハンクス主演。世界8000万部超のベストセラー原作。歴史・美術・宗教の謎解き要素が映画でも堪能できる。",
    recommendedFor: "美術史・宗教史の謎解きに興味がある方に。映画を見て原作のより詳細な謎解き過程を追いたい方に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "percy-jackson-novel-overseas",
    mediaTitle: "パーシー・ジャクソンと神々の戦い",
    mediaType: "overseas",
    mediaYear: 2010,
    originalExists: true,
    originalTitle: "パーシー・ジャクソンとオリンポスの神々",
    originalAuthor: "リック・リオーダン",
    originalType: "novel",
    adaptationLabel: "小説原作",
    description: "ギリシャ神話の神々が現代に実在する世界で、ポセイドンの息子の少年が神話の世界で活躍するファンタジーの映画化。",
    searchAliases: ["パーシー・ジャクソン", "Percy Jackson", "リック・リオーダン", "ギリシャ神話"],
    adaptationNotes: "映画版2作の後、2024年にディズニープラスでドラマシリーズも制作。原作はYA小説の大ヒットシリーズ全5巻。",
    recommendedFor: "ギリシャ神話に興味がある方や子どもから若い大人向けのファンタジーを探している方に。",
    workId: "",
    thumbnailUrl: "",
  },

  // === Part2追加: movie（12件）===
  {
    id: "josee-tiger-movie",
    mediaTitle: "ジョゼと虎と魚たち",
    mediaType: "movie",
    mediaYear: 2003,
    originalExists: true,
    originalTitle: "ジョゼと虎と魚たち",
    originalAuthor: "田辺聖子",
    originalType: "novel",
    adaptationLabel: "映画化",
    description: "車椅子の少女ジョゼと大学生の恋を描くラブストーリー。田辺聖子の短編小説が原作の名作映画。",
    searchAliases: ["ジョゼ", "田辺聖子", "妻夫木聡", "上野樹里"],
    adaptationNotes: "映画は原作短編を長編化し、より深い感情描写に仕上げた。2020年にはアニメ映画版も制作。原作の軽妙さとは異なり映画版はシリアスなトーン。",
    recommendedFor: "恋愛映画から田辺聖子の小説世界に触れたい方。切ない恋愛物語が好きな人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "sekai-chuushin-movie",
    mediaTitle: "世界の中心で、愛をさけぶ",
    mediaType: "movie",
    mediaYear: 2004,
    originalExists: true,
    originalTitle: "世界の中心で、愛をさけぶ",
    originalAuthor: "片山恭一",
    originalType: "novel",
    adaptationLabel: "映画化",
    description: "白血病で亡くなった恋人の思い出を辿る純愛映画。2001年刊の小説が大ベストセラーとなり映画化・ドラマ化された。",
    searchAliases: ["セカチュー", "片山恭一", "長澤まさみ", "山田孝之"],
    adaptationNotes: "原作の静謐な文体が映画では映像美で表現。テレビドラマ版（TBS）との比較も面白い。原作は回想構造で語られる叙情的な純愛小説。",
    recommendedFor: "純愛映画から文学に入りたい方。2000年代の青春恋愛作品を楽しみたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "hankyu-densha-movie",
    mediaTitle: "阪急電車 片道15分の奇跡",
    mediaType: "movie",
    mediaYear: 2011,
    originalExists: true,
    originalTitle: "阪急電車",
    originalAuthor: "有川浩",
    originalType: "novel",
    adaptationLabel: "映画化",
    description: "阪急今津線の乗客たちの人間模様を描く群像劇。有川浩の連作短編小説が原作のハートウォーミング映画。",
    searchAliases: ["阪急電車", "有川浩", "中谷美紀", "宮本信子"],
    adaptationNotes: "原作の連作短編構造を映画用に再構成。キャラクターの掘り下げが映画と原作で異なる。原作は各乗客のエピソードがより詳細に描かれる。",
    recommendedFor: "日常の温かいドラマが好きな方。有川浩の読みやすい文体から小説を始めたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "birigal-movie",
    mediaTitle: "ビリギャル",
    mediaType: "movie",
    mediaYear: 2015,
    originalExists: true,
    originalTitle: "学年ビリのギャルが1年で偏差値を40上げて慶應大学に現役合格した話",
    originalAuthor: "坪田信貴",
    originalType: "nonfiction",
    adaptationLabel: "映画化",
    description: "学年ビリのギャルが慶應大学に合格した実話を映画化。坪田信貴著のベストセラーノンフィクションが原作。",
    searchAliases: ["ビリギャル", "坪田信貴", "有村架純", "慶應"],
    adaptationNotes: "映画は原作を忠実に再現しつつドラマチックに演出。実話ベースの原作の方が細かいエピソードが豊富。映画は感動的な教育ドラマとして高く評価された。",
    recommendedFor: "受験や勉強に悩む学生・親御さん。実話に基づく成長ストーリーが好きな人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "coffee-samenai-movie",
    mediaTitle: "コーヒーが冷めないうちに",
    mediaType: "movie",
    mediaYear: 2018,
    originalExists: true,
    originalTitle: "コーヒーが冷めないうちに",
    originalAuthor: "川口俊和",
    originalType: "novel",
    adaptationLabel: "映画化",
    description: "過去に戻れる不思議な喫茶店を舞台にした感動のSFファンタジー。川口俊和の舞台発小説が原作。",
    searchAliases: ["コーヒーが冷めないうちに", "川口俊和", "有村架純", "タイムリープ"],
    adaptationNotes: "映画はオムニバス構造の原作4話を一本に再構成。映像化で美しい喫茶店の雰囲気が表現された。原作は続編も多数刊行されているシリーズ。",
    recommendedFor: "泣ける映画から小説に入りたい方。タイムリープ×人情話が好きな人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "yoka-me-no-semi-movie",
    mediaTitle: "八日目の蝉",
    mediaType: "movie",
    mediaYear: 2011,
    originalExists: true,
    originalTitle: "八日目の蝉",
    originalAuthor: "角田光代",
    originalType: "novel",
    adaptationLabel: "映画化",
    description: "赤ちゃんを連れ去った女と育てられた子の数奇な運命を描く社会派ドラマ。角田光代の長編小説が原作。",
    searchAliases: ["八日目の蝉", "角田光代", "井上真央", "永作博美"],
    adaptationNotes: "映画は原作の複雑な時系列を整理して映像化。NHKドラマ版との比較も興味深い。原作は母性と女性の生き方をより深く掘り下げている。",
    recommendedFor: "社会問題をテーマにした骨太な物語が好きな方。角田光代の人間ドラマに興味がある人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "tottochan-movie",
    mediaTitle: "窓ぎわのトットちゃん",
    mediaType: "movie",
    mediaYear: 2023,
    originalExists: true,
    originalTitle: "窓ぎわのトットちゃん",
    originalAuthor: "黒柳徹子",
    originalType: "essay",
    adaptationLabel: "アニメ映画化",
    description: "黒柳徹子の自伝的エッセイを2023年にアニメ映画化。戦前の自由な学校「トモエ学園」での日々を描く。",
    searchAliases: ["トットちゃん", "黒柳徹子", "トモエ学園", "小林先生"],
    adaptationNotes: "1981年刊行の原作を初めてアニメ映画化。原作は子どもの目線で語られる温かい自伝エッセイ。映画は戦時下の時代背景を丁寧に描写している。",
    recommendedFor: "子育て中の親御さんや教育に関心がある方。懐かしい昭和の時代を感じたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "orange-manga-movie",
    mediaTitle: "orange -オレンジ-",
    mediaType: "movie",
    mediaYear: 2015,
    originalExists: true,
    originalTitle: "orange",
    originalAuthor: "高野苺",
    originalType: "manga",
    adaptationLabel: "映画化",
    description: "未来の自分から届いた手紙を基に転校生を救おうとする青春恋愛漫画の実写映画化。",
    searchAliases: ["orange オレンジ", "高野苺", "山崎賢人", "土屋太鳳"],
    adaptationNotes: "原作漫画の世界観を忠実に映像化。2016年にアニメ版も制作。原作では手紙の詳細なやりとりがより丁寧に描かれている。",
    recommendedFor: "青春・タイムトラベル系の感動物語が好きな方。漫画から映画・アニメと楽しみたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "kono-sekai-katachi-movie",
    mediaTitle: "この世界の片隅に",
    mediaType: "movie",
    mediaYear: 2016,
    originalExists: true,
    originalTitle: "この世界の片隅に",
    originalAuthor: "こうの史代",
    originalType: "manga",
    adaptationLabel: "アニメ映画化",
    description: "戦時中の広島・呉を舞台に一般市民の日常を描くアニメ映画。片渕須直監督の傑作。",
    searchAliases: ["この世界の片隅に", "こうの史代", "のん", "片渕須直"],
    adaptationNotes: "原作漫画に極めて忠実に制作。2019年に「この世界の（さらにいくつもの）片隅に」としてシーンを追加した版も公開。原作漫画は日常描写がより詳細。",
    recommendedFor: "戦争の悲惨さではなく日常の尊さを描いた作品を求める方。漫画原作も読み比べたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "taiyou-ugakanai-movie",
    mediaTitle: "太陽は動かない",
    mediaType: "movie",
    mediaYear: 2021,
    originalExists: true,
    originalTitle: "太陽は動かない",
    originalAuthor: "吉田修一",
    originalType: "novel",
    adaptationLabel: "映画化",
    description: "産業スパイとして暗躍するエージェントたちの国際サスペンス。吉田修一のベストセラー小説が原作。",
    searchAliases: ["太陽は動かない", "吉田修一", "藤原竜也", "竹内涼真"],
    adaptationNotes: "映画は原作の続編「森は知っている」のエピソードも取り込み再構成。原作2冊を映画1本に凝縮しているため原作の方がキャラクター描写が深い。",
    recommendedFor: "国際スパイアクションが好きな方。吉田修一の骨太なエンタメ小説を探している人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "platinum-data-movie",
    mediaTitle: "プラチナデータ",
    mediaType: "movie",
    mediaYear: 2013,
    originalExists: true,
    originalTitle: "プラチナデータ",
    originalAuthor: "東野圭吾",
    originalType: "novel",
    adaptationLabel: "映画化",
    description: "DNAデータによる犯罪捜査と多重人格をテーマにした近未来サスペンス。東野圭吾原作。",
    searchAliases: ["プラチナデータ", "東野圭吾", "二宮和也", "豊川悦司"],
    adaptationNotes: "原作のSF的設定と倫理的テーマを映画化。二宮和也の二役演技が話題に。原作は人物の心理描写がより詳細で技術的・倫理的議論が深い。",
    recommendedFor: "東野圭吾のSF×ミステリが好きな方。DNAや個人情報の倫理問題に興味がある人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "i-am-a-hero-movie",
    mediaTitle: "アイアムアヒーロー",
    mediaType: "movie",
    mediaYear: 2016,
    originalExists: true,
    originalTitle: "アイアムアヒーロー",
    originalAuthor: "花沢健吾",
    originalType: "manga",
    adaptationLabel: "映画化",
    description: "ゾンビパニックを舞台に内気な漫画家が生き残りをかけて戦う実写ホラー映画。花沢健吾の漫画が原作。",
    searchAliases: ["アイアムアヒーロー", "花沢健吾", "大泉洋", "有村架純"],
    adaptationNotes: "原作漫画の前半部分を中心に映画化。リアルなゾンビ描写と心理描写が特徴的。原作は全22巻の長編で映画で省かれたエピソードが多数ある。",
    recommendedFor: "ゾンビ・サバイバルホラーが好きな方。日本のパニック漫画の傑作を読みたい人に。",
    workId: "",
    thumbnailUrl: "",
  },

  // === Part2追加: drama（13件）===
  {
    id: "shinzanmono-novel-drama",
    mediaTitle: "新参者",
    mediaType: "drama",
    mediaYear: 2010,
    originalExists: true,
    originalTitle: "新参者",
    originalAuthor: "東野圭吾",
    originalType: "novel",
    adaptationLabel: "テレビドラマ化",
    description: "下町・人形町を舞台に加賀恭一郎刑事が事件の真相に迫る連作推理ドラマ。東野圭吾の人気シリーズ。",
    searchAliases: ["新参者", "加賀恭一郎", "東野圭吾", "阿部寛"],
    adaptationNotes: "ドラマは原作の連作短編構造を踏襲。テレビ版限定のオリジナルシーンも加えられている。原作は各店主たちとの人情交流が丁寧に描かれる。",
    recommendedFor: "人情ミステリーが好きな方。東野圭吾の加賀恭一郎シリーズを読んでみたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "sora-tobu-kohoushitsu-novel-drama",
    mediaTitle: "空飛ぶ広報室",
    mediaType: "drama",
    mediaYear: 2013,
    originalExists: true,
    originalTitle: "空飛ぶ広報室",
    originalAuthor: "有川浩",
    originalType: "novel",
    adaptationLabel: "テレビドラマ化",
    description: "航空自衛隊の広報室を舞台にしたラブコメディ。有川浩の航空×恋愛小説をTBS系でドラマ化。",
    searchAliases: ["空飛ぶ広報室", "有川浩", "綾野剛", "新垣結衣"],
    adaptationNotes: "原作に概ね忠実なドラマ化。有川浩らしいユーモアと恋愛描写が映像でも再現された。原作は自衛隊の細かい描写や登場人物の内面がより丁寧に描かれる。",
    recommendedFor: "有川浩ファン・ラブコメ好きな方。自衛隊の知られざる仕事に興味がある人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "nokosaido-game-novel-drama",
    mediaTitle: "ノーサイドゲーム",
    mediaType: "drama",
    mediaYear: 2019,
    originalExists: true,
    originalTitle: "ノーサイドゲーム",
    originalAuthor: "池井戸潤",
    originalType: "novel",
    adaptationLabel: "テレビドラマ化",
    description: "企業ラグビー部の再建をかけたビジネスドラマ。池井戸潤の熱血エンタメ小説をTBS系でドラマ化。",
    searchAliases: ["ノーサイドゲーム", "池井戸潤", "大泉洋", "ラグビー"],
    adaptationNotes: "ラグビーワールドカップ2019年に合わせて放送。原作の池井戸潤らしい企業の不条理と熱い逆転劇が忠実に再現されている。",
    recommendedFor: "池井戸潤ファン・スポーツドラマが好きな方。ラグビーを題材にした熱い物語を楽しみたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "rikuou-novel-drama",
    mediaTitle: "陸王",
    mediaType: "drama",
    mediaYear: 2017,
    originalExists: true,
    originalTitle: "陸王",
    originalAuthor: "池井戸潤",
    originalType: "novel",
    adaptationLabel: "テレビドラマ化",
    description: "老舗足袋屋がランニングシューズ開発に挑む中小企業の再生ドラマ。池井戸潤のベストセラー小説が原作。",
    searchAliases: ["陸王", "池井戸潤", "役所広司", "竹内涼真"],
    adaptationNotes: "TBS系で放送し高視聴率を記録。原作の細かい製造業のプロセスやビジネス描写が丁寧に映像化された。原作は会社再生の苦難がより詳細に描かれている。",
    recommendedFor: "池井戸潤の企業再生ドラマが好きな方。ものづくりの情熱と中小企業の苦労に共感できる人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "juhan-shuttai-manga-drama",
    mediaTitle: "重版出来！",
    mediaType: "drama",
    mediaYear: 2016,
    originalExists: true,
    originalTitle: "重版出来！",
    originalAuthor: "松田奈緒子",
    originalType: "manga",
    adaptationLabel: "テレビドラマ化",
    description: "漫画編集者の仕事と情熱を描く業界ドラマ。松田奈緒子の漫画をTBS系でドラマ化。黒木華主演。",
    searchAliases: ["重版出来", "松田奈緒子", "黒木華", "漫画編集"],
    adaptationNotes: "原作漫画の職人的な世界観と感動エピソードを忠実に再現。ドラマでは1クールに原作を再構成。原作は各漫画家や書店員など多角的な視点で業界を描く。",
    recommendedFor: "漫画・出版業界に興味がある方。仕事へのひたむきさに共感できる人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "nagi-no-oyasumi-manga-drama",
    mediaTitle: "凪のお暇",
    mediaType: "drama",
    mediaYear: 2019,
    originalExists: true,
    originalTitle: "凪のお暇",
    originalAuthor: "コナリミサト",
    originalType: "manga",
    adaptationLabel: "テレビドラマ化",
    description: "空気を読みすぎた女性が仕事も恋人も捨てて自分らしく生きる姿を描くドラマ。コナリミサト漫画原作。",
    searchAliases: ["凪のお暇", "コナリミサト", "黒木華", "中村倫也"],
    adaptationNotes: "TBS系放送。原作漫画のゆるやかなテンポと主人公の成長が丁寧に映像化。ドラマオリジナルのシーンも追加されている。原作は現在も連載中。",
    recommendedFor: "自分らしさを模索する人・ゆるやかな成長物語が好きな方。コナリミサトの個性的な漫画を楽しみたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "shojiki-fudosan-manga-drama",
    mediaTitle: "正直不動産",
    mediaType: "drama",
    mediaYear: 2022,
    originalExists: true,
    originalTitle: "正直不動産",
    originalAuthor: "大谷アキラ",
    originalType: "manga",
    adaptationLabel: "テレビドラマ化",
    description: "嘘がつけなくなった不動産営業マンが本音で仕事をするコメディドラマ。大谷アキラの漫画が原作。",
    searchAliases: ["正直不動産", "大谷アキラ", "山下智久", "不動産"],
    adaptationNotes: "NHK放送。原作漫画の不動産業界の知識と笑いを忠実に映像化。ドラマヒット後にシーズン2も制作。原作は業界の裏事情をわかりやすく解説している。",
    recommendedFor: "不動産業界の仕組みを知りたい方。仕事コメディが好きな人。家の購入・賃貸を考えている人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "hanako-to-anne-nonfiction-drama",
    mediaTitle: "花子とアン",
    mediaType: "drama",
    mediaYear: 2014,
    originalExists: true,
    originalTitle: "アンのゆりかご—村岡花子の生涯",
    originalAuthor: "村岡恵理",
    originalType: "nonfiction",
    adaptationLabel: "NHK連続テレビ小説",
    description: "「赤毛のアン」翻訳者・村岡花子の生涯を描くNHK朝ドラ。村岡恵理のノンフィクション伝記が原作。",
    searchAliases: ["花子とアン", "村岡花子", "赤毛のアン", "吉高由里子"],
    adaptationNotes: "NHK朝ドラとして放送し好評を博した。原作は村岡花子の翻訳人生と女性としての生き方を丁寧に記録したノンフィクション。ドラマでは友情と恋愛も強調。",
    recommendedFor: "「赤毛のアン」が好きな方。昭和の女性の生き方や翻訳文学の歴史に興味がある人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "ichirittoru-no-namida-drama",
    mediaTitle: "1リットルの涙",
    mediaType: "drama",
    mediaYear: 2005,
    originalExists: true,
    originalTitle: "1リットルの涙",
    originalAuthor: "木藤亜也",
    originalType: "nonfiction",
    adaptationLabel: "テレビドラマ化",
    description: "脊髄小脳変性症と闘った少女の実話をドラマ化。木藤亜也の闘病日記が原作。沢尻エリカ主演。",
    searchAliases: ["1リットルの涙", "木藤亜也", "沢尻エリカ", "難病"],
    adaptationNotes: "原作は実際の闘病日記をまとめたもの。ドラマではドラマチックな演出や恋愛要素が加えられている。原作の真摯な日記は映像とは異なるリアルな感動がある。",
    recommendedFor: "実話に基づく感動物語が好きな方。生きることの意味を考えたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "wakako-zake-manga-drama",
    mediaTitle: "ワカコ酒",
    mediaType: "drama",
    mediaYear: 2014,
    originalExists: true,
    originalTitle: "ワカコ酒",
    originalAuthor: "新久千映",
    originalType: "manga",
    adaptationLabel: "テレビドラマ化",
    description: "OLがひとり居酒屋で美食とお酒を楽しむグルメ漫画のドラマ化。武田梨奈主演の人気深夜ドラマ。",
    searchAliases: ["ワカコ酒", "新久千映", "武田梨奈", "一人飲み"],
    adaptationNotes: "テレビ東京の深夜ドラマとして人気を博しシリーズが続く。原作漫画の美食描写と「ぷしゅ〜」のゆるやかな世界観が忠実に再現。",
    recommendedFor: "グルメ・お酒好きの方。一人の時間を大切にしたい女性に。ゆるやかな癒し漫画を探している人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "tomodachi-game-manga-drama",
    mediaTitle: "友達ゲーム",
    mediaType: "drama",
    mediaYear: 2022,
    originalExists: true,
    originalTitle: "友達ゲーム",
    originalAuthor: "山田鐘人",
    originalType: "manga",
    adaptationLabel: "テレビドラマ化",
    description: "5人の友達が借金を返すため心理ゲームに参加するサスペンス。山田鐘人の漫画が原作のフジテレビドラマ。",
    searchAliases: ["友達ゲーム", "山田鐘人", "赤楚衛二", "心理戦"],
    adaptationNotes: "フジテレビ系で放送。原作漫画の複雑な心理戦と裏切りの展開を映像化。原作は現在も連載中で、ドラマ化は前半部分を中心に構成。",
    recommendedFor: "心理戦・サスペンスが好きな方。裏切りと信頼をテーマにしたスリリングな物語を楽しみたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "hibana-novel-drama",
    mediaTitle: "火花",
    mediaType: "drama",
    mediaYear: 2016,
    originalExists: true,
    originalTitle: "火花",
    originalAuthor: "又吉直樹",
    originalType: "novel",
    adaptationLabel: "Netflixドラマ化",
    description: "お笑い芸人の師弟関係と夢を描く芥川賞受賞作品のNetflixドラマ化。又吉直樹の自伝的小説。",
    searchAliases: ["火花", "又吉直樹", "林遣都", "お笑い", "芥川賞"],
    adaptationNotes: "Netflixオリジナルとして全15話で配信。原作の詩的な文体と芸人の心情描写が映像で丁寧に再現された。原作は短編ながら密度の高い文学作品。",
    recommendedFor: "芥川賞文学・お笑い芸人の世界に興味がある方。又吉直樹の独特な文体を楽しみたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "algernon-japan-drama",
    mediaTitle: "アルジャーノンに花束を（日本版）",
    mediaType: "drama",
    mediaYear: 2015,
    originalExists: true,
    originalTitle: "アルジャーノンに花束を",
    originalAuthor: "ダニエル・キイス",
    originalType: "novel",
    adaptationLabel: "テレビドラマ化",
    description: "知的障害を持つ青年が手術で天才になる米国小説の日本版ドラマ化。山下智久主演。TBS系。",
    searchAliases: ["アルジャーノンに花束を", "ダニエル・キイス", "山下智久", "Daniel Keyes"],
    adaptationNotes: "1966年刊のアメリカ小説を日本の現代に舞台を移してドラマ化。原作は知性と心の成長・喪失を日記形式で描いた名著。日本版は設定を変えつつ本質を継承。",
    recommendedFor: "感動的な海外文学を探している方。人間の知性・尊厳をテーマにした深い物語が好きな人に。",
    workId: "",
    thumbnailUrl: "",
  },

  // === Part2追加: anime（15件）===
  {
    id: "sword-art-online-novel-anime",
    mediaTitle: "ソードアート・オンライン",
    mediaType: "anime",
    mediaYear: 2012,
    originalExists: true,
    originalTitle: "ソードアート・オンライン",
    originalAuthor: "川原礫",
    originalType: "novel",
    adaptationLabel: "TVアニメ化",
    description: "完全仮想現実RPGに閉じ込められた少年キリトの冒険を描く異世界アクション。川原礫のライトノベルが原作。",
    searchAliases: ["SAO", "ソードアート", "川原礫", "キリト", "アスナ"],
    adaptationNotes: "原作ライトノベルを忠実にアニメ化。アニメオリジナルのシーンも追加されている。原作は月刊ウォーカーへの連載から始まり現在も続く大長編シリーズ。",
    recommendedFor: "VRゲームや異世界ファンタジーが好きな方。アニメを見てキャラクターをより深く知りたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "hunter-x-hunter-manga-anime",
    mediaTitle: "HUNTER×HUNTER",
    mediaType: "anime",
    mediaYear: 2011,
    originalExists: true,
    originalTitle: "HUNTER×HUNTER",
    originalAuthor: "冨樫義博",
    originalType: "manga",
    adaptationLabel: "TVアニメ化",
    description: "ハンターを目指す少年ゴンの冒険と成長を描く少年漫画の2011年版アニメ化作品。",
    searchAliases: ["ハンターハンター", "冨樫義博", "ゴン", "キルア", "念能力"],
    adaptationNotes: "1999年版に続く2作目のアニメ化。漫画原作に忠実で、特にキメラアント編が高く評価される。原作は不定期連載中だが漫画としての完成度が極めて高い。",
    recommendedFor: "壮大な冒険ファンタジーが好きな方。個性豊かな能力バトルと深い人間ドラマを楽しみたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "gintama-manga-anime",
    mediaTitle: "銀魂",
    mediaType: "anime",
    mediaYear: 2006,
    originalExists: true,
    originalTitle: "銀魂",
    originalAuthor: "空知英秋",
    originalType: "manga",
    adaptationLabel: "TVアニメ化",
    description: "江戸時代に宇宙人が来た世界を舞台にした万事屋・銀時の活躍を描くギャグ×シリアスアニメ。",
    searchAliases: ["銀魂", "空知英秋", "坂田銀時", "万事屋"],
    adaptationNotes: "テレビ東京系で367話放送。ギャグと熱いシリアスパートのバランスが特徴。原作漫画全77巻は完結済みで、アニメは原作に概ね忠実に展開した。",
    recommendedFor: "ギャグ×シリアスの作品が好きな方。江戸×SF×時代劇という独特の世界観を楽しみたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "nanatsu-taizai-manga-anime",
    mediaTitle: "七つの大罪",
    mediaType: "anime",
    mediaYear: 2014,
    originalExists: true,
    originalTitle: "七つの大罪",
    originalAuthor: "鈴木央",
    originalType: "manga",
    adaptationLabel: "TVアニメ化",
    description: "かつて王国を裏切ったとされる伝説の騎士団と王女の冒険を描くファンタジー。鈴木央の漫画が原作。",
    searchAliases: ["七つの大罪", "鈴木央", "メリオダス", "エリザベス"],
    adaptationNotes: "NHK Eテレ放送後、Netflixでも全世界配信。原作漫画全41巻は完結済み。アニメは原作の壮大なファンタジー世界を映像化し人気を博した。",
    recommendedFor: "王道ファンタジーアクションが好きな方。恋愛と冒険を両立した大河ファンタジーを楽しみたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "honzuki-no-gekokujou-novel-anime",
    mediaTitle: "本好きの下剋上",
    mediaType: "anime",
    mediaYear: 2019,
    originalExists: true,
    originalTitle: "本好きの下剋上",
    originalAuthor: "香月美夜",
    originalType: "novel",
    adaptationLabel: "TVアニメ化",
    description: "本を愛する司書が異世界に転生し本作りから始める異世界ファンタジー。香月美夜の小説が原作。",
    searchAliases: ["本好きの下剋上", "香月美夜", "マイン", "本好き"],
    adaptationNotes: "「小説家になろう」発のWeb小説が書籍化されアニメ化。原作は5部構成の長編シリーズ。アニメは第1部から丁寧にアニメ化が進んでいる。",
    recommendedFor: "読書好きの方・異世界転生ファンタジーが好きな方。知的好奇心と成長を描いた物語を楽しみたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "tate-no-yuusha-novel-anime",
    mediaTitle: "盾の勇者の成り上がり",
    mediaType: "anime",
    mediaYear: 2019,
    originalExists: true,
    originalTitle: "盾の勇者の成り上がり",
    originalAuthor: "アネコユサギ",
    originalType: "novel",
    adaptationLabel: "TVアニメ化",
    description: "四聖勇者として異世界に召喚されたのに裏切られた盾の勇者の復讐と成長の物語。",
    searchAliases: ["盾の勇者", "アネコユサギ", "岩谷尚文", "ラフタリア"],
    adaptationNotes: "「小説家になろう」発の異世界小説をアニメ化。原作シリーズは全22巻。勇者が冒頭から理不尽な扱いを受ける展開が話題となった。",
    recommendedFor: "異世界転生・復讐物語が好きな方。逆境から這い上がるサクセスストーリーを楽しみたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "yojo-senki-novel-anime",
    mediaTitle: "幼女戦記",
    mediaType: "anime",
    mediaYear: 2017,
    originalExists: true,
    originalTitle: "幼女戦記",
    originalAuthor: "カルロ・ゼン",
    originalType: "novel",
    adaptationLabel: "TVアニメ化",
    description: "社畜サラリーマンが幼女に転生し架空の世界大戦を戦う異世界戦記ファンタジー。",
    searchAliases: ["幼女戦記", "カルロ・ゼン", "ターニャ・デグレチャフ"],
    adaptationNotes: "「小説家になろう」発のWeb小説をアニメ化。映画版も2019年に公開。原作小説の深い戦略思考と世界観を映像で迫力ある戦闘シーンと共に描く。",
    recommendedFor: "ミリタリー・戦記ファンタジーが好きな方。皮肉の効いたダークファンタジーを楽しみたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "made-in-abyss-manga-anime",
    mediaTitle: "メイドインアビス",
    mediaType: "anime",
    mediaYear: 2017,
    originalExists: true,
    originalTitle: "メイドインアビス",
    originalAuthor: "つくしあきひと",
    originalType: "manga",
    adaptationLabel: "TVアニメ化",
    description: "巨大な縦穴「アビス」を探索する少女と少年の冒険を描くダークファンタジー。つくしあきひと漫画原作。",
    searchAliases: ["メイドインアビス", "つくしあきひと", "リコ", "レグ", "ナナチ"],
    adaptationNotes: "可愛い見た目とは裏腹にシリアスで残酷な展開が特徴。映画版も2作制作。原作漫画は現在も連載中で、アニメは原作の序盤から中盤を映像化。",
    recommendedFor: "ダークファンタジー・冒険物語が好きな方。独特の世界観と感動的なキャラクター描写を楽しみたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "dororo-manga-anime",
    mediaTitle: "どろろ（2019年版）",
    mediaType: "anime",
    mediaYear: 2019,
    originalExists: true,
    originalTitle: "どろろ",
    originalAuthor: "手塚治虫",
    originalType: "manga",
    adaptationLabel: "TVアニメ化",
    description: "体の48か所を魔物に奪われた侍・百鬼丸が体を取り戻す旅を描く手塚治虫の傑作漫画の2019年版リメイク。",
    searchAliases: ["どろろ", "手塚治虫", "百鬼丸", "どろろ2019"],
    adaptationNotes: "1969年のテレビアニメに続きMAPPA制作でリメイク。原作漫画未完の部分もアニメが独自解釈で完結させた。現代的な映像美で手塚作品を再解釈した傑作。",
    recommendedFor: "手塚治虫作品に興味がある方。時代劇×ダークファンタジーを楽しみたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "mob-psycho-manga-anime",
    mediaTitle: "モブサイコ100",
    mediaType: "anime",
    mediaYear: 2016,
    originalExists: true,
    originalTitle: "モブサイコ100",
    originalAuthor: "ONE",
    originalType: "manga",
    adaptationLabel: "TVアニメ化",
    description: "強大な霊能力を持つ少年・モブの成長を描くアクション×ヒューマンドラマ。ワンパンマン作者ONEの漫画原作。",
    searchAliases: ["モブサイコ", "ONE", "影山茂夫", "霊能力"],
    adaptationNotes: "ボンズ制作のアニメは独特の作画スタイルで高い評価を得た。原作はシンプルな絵柄ながら深い人間ドラマが特徴。全3シーズン放送済み。",
    recommendedFor: "超能力×成長物語が好きな方。人間の感情と強さについて考えさせられる作品を探している人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "one-punch-man-manga-anime",
    mediaTitle: "ワンパンマン",
    mediaType: "anime",
    mediaYear: 2015,
    originalExists: true,
    originalTitle: "ワンパンマン",
    originalAuthor: "ONE / 村田雄介",
    originalType: "manga",
    adaptationLabel: "TVアニメ化",
    description: "一撃で全ての敵を倒せるヒーロー・サイタマの活躍を描くパロディアクション漫画のアニメ化。",
    searchAliases: ["ワンパンマン", "ONE", "村田雄介", "サイタマ", "ジェノス"],
    adaptationNotes: "Web漫画（ONE原作）をリメイクした村田雄介版漫画をアニメ化。第1期はMadhouse制作で映像クオリティが話題に。原作Web漫画は現在も更新中。",
    recommendedFor: "爽快なアクションとシュールなコメディが好きな方。ヒーロー物のパロディを楽しみたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "yuyu-hakusho-manga-anime",
    mediaTitle: "幽☆遊☆白書",
    mediaType: "anime",
    mediaYear: 1992,
    originalExists: true,
    originalTitle: "幽☆遊☆白書",
    originalAuthor: "冨樫義博",
    originalType: "manga",
    adaptationLabel: "TVアニメ化",
    description: "不良少年・浦飯幽助が死後に霊界探偵として活躍するバトルファンタジー。2024年にNetflixが実写化も話題に。",
    searchAliases: ["幽遊白書", "冨樫義博", "浦飯幽助", "戸愚呂兄弟"],
    adaptationNotes: "フジテレビ系で1992〜95年放送の名作。2024年にNetflixで実写ドラマも制作。原作漫画全19巻は完結済み。アニメは暗黒武術会編まで忠実に描く。",
    recommendedFor: "90年代バトル漫画の傑作に触れたい方。冨樫義博の世界観が好きなHUNTER×HUNTERファンに。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "ranking-of-kings-manga-anime",
    mediaTitle: "王様ランキング",
    mediaType: "anime",
    mediaYear: 2021,
    originalExists: true,
    originalTitle: "王様ランキング",
    originalAuthor: "十日草輔",
    originalType: "manga",
    adaptationLabel: "TVアニメ化",
    description: "耳が聞こえず力も弱い王子・ボッジが真の王を目指す感動のダークファンタジー。Webマンガ発の人気作。",
    searchAliases: ["王様ランキング", "十日草輔", "ボッジ", "カゲ"],
    adaptationNotes: "WebマンガからWIT STUDIO制作でアニメ化し高い評価を受けた。原作は独特のゆるい絵柄と深い感動ストーリーのギャップが特徴。",
    recommendedFor: "感動のファンタジーが好きな方。見た目に惑わされない努力と成長の物語を楽しみたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "ao-no-exorcist-manga-anime",
    mediaTitle: "青の祓魔師",
    mediaType: "anime",
    mediaYear: 2011,
    originalExists: true,
    originalTitle: "青の祓魔師",
    originalAuthor: "加藤和恵",
    originalType: "manga",
    adaptationLabel: "TVアニメ化",
    description: "悪魔の子として生まれた少年が祓魔師を目指す人気ファンタジーアクション。加藤和恵の漫画が原作。",
    searchAliases: ["青の祓魔師", "加藤和恵", "奥村燐", "悪魔", "エクソシスト"],
    adaptationNotes: "第1期はオリジナル展開で完結。2017年放送の京都不浄王篇は原作に沿った展開。原作漫画はジャンプスクエア連載中で現在も続く大長編。",
    recommendedFor: "ダークファンタジーアクションが好きな方。悪魔と人間の狭間で生きるキャラクターの葛藤を楽しみたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "kuroshitsuji-manga-anime",
    mediaTitle: "黒執事",
    mediaType: "anime",
    mediaYear: 2008,
    originalExists: true,
    originalTitle: "黒執事",
    originalAuthor: "枢やな",
    originalType: "manga",
    adaptationLabel: "TVアニメ化",
    description: "19世紀英国貴族の少年と悪魔の執事セバスチャンの主従関係を描くダーク・ファンタジー。",
    searchAliases: ["黒執事", "枢やな", "セバスチャン", "シエル"],
    adaptationNotes: "第1期・第2期・Book of Circus等シリーズ化。2024年には新アニメも制作。原作漫画はスクウェア・エニックスで連載中。アニメと原作で一部ストーリーが異なる。",
    recommendedFor: "ゴシック・ダークファンタジーが好きな方。英国貴族の雰囲気と謎解きを楽しみたい人に。",
    workId: "",
    thumbnailUrl: "",
  },

  // === Part2追加: overseas（10件）===
  {
    id: "hunger-games-novel-overseas",
    mediaTitle: "ハンガー・ゲーム",
    mediaType: "overseas",
    mediaYear: 2012,
    originalExists: true,
    originalTitle: "ハンガー・ゲーム",
    originalAuthor: "スーザン・コリンズ",
    originalType: "novel",
    adaptationLabel: "映画化",
    description: "ディストピア国家で殺し合いのゲームに参加させられる少女カットニスの戦いを描くYA映画。全4作。",
    searchAliases: ["ハンガーゲーム", "カットニス", "スーザン・コリンズ", "The Hunger Games"],
    adaptationNotes: "原作三部作を映画4作に分割。前日譚小説「バラードと蛇の歌」も2023年に映画化。原作は一人称視点で主人公の内面が豊かに描かれる。",
    recommendedFor: "ディストピア系YA作品が好きな方。女性主人公の成長と革命をテーマにした物語を楽しみたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "twilight-novel-overseas",
    mediaTitle: "トワイライト〜初恋〜",
    mediaType: "overseas",
    mediaYear: 2008,
    originalExists: true,
    originalTitle: "トワイライト",
    originalAuthor: "ステファニー・メイヤー",
    originalType: "novel",
    adaptationLabel: "映画化",
    description: "普通の女子高生とヴァンパイアの禁断の恋を描くロマンティック・ファンタジー映画。全5作シリーズ。",
    searchAliases: ["トワイライト", "ステファニー・メイヤー", "ベラ", "エドワード", "Twilight"],
    adaptationNotes: "原作4巻を映画5作に分割。世界的ベストセラーで10代女性を中心に爆発的ヒット。原作の繊細な恋愛描写と内面独白が映画では映像的ロマンスとして表現された。",
    recommendedFor: "ロマンティック・ファンタジーが好きな方。ヴァンパイアと人間の恋愛を扱った大人気YAシリーズを読みたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "murder-orient-express-movie-overseas",
    mediaTitle: "オリエント急行殺人事件",
    mediaType: "overseas",
    mediaYear: 2017,
    originalExists: true,
    originalTitle: "オリエント急行殺人事件",
    originalAuthor: "アガサ・クリスティ",
    originalType: "novel",
    adaptationLabel: "映画化",
    description: "雪で立ち往生した豪華列車内で起きた殺人をポワロが解く名作ミステリーの2017年版映画化。",
    searchAliases: ["オリエント急行", "アガサ・クリスティ", "ポワロ", "ケネス・ブラナー"],
    adaptationNotes: "1974年版映画・テレビドラマ版に続く映画化。原作は1934年刊行のクリスティ代表作。意外な結末が有名。映画版は豪華俳優陣と映像美が見どころ。",
    recommendedFor: "本格ミステリーが好きな方。アガサ・クリスティの古典的名作から推理小説を始めたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "big-little-lies-novel-overseas",
    mediaTitle: "ビッグ・リトル・ライズ",
    mediaType: "overseas",
    mediaYear: 2017,
    originalExists: true,
    originalTitle: "ビッグ・リトル・ライズ",
    originalAuthor: "リアーン・モリアーティ",
    originalType: "novel",
    adaptationLabel: "HBOドラマ化",
    description: "小さな海辺の町の女性たちの友情と秘密を描くHBOリミテッドシリーズ。エミー賞複数受賞作品。",
    searchAliases: ["ビッグリトルライズ", "リアーン・モリアーティ", "ニコール・キッドマン", "HBO"],
    adaptationNotes: "原作オーストラリア小説をアメリカ舞台にドラマ化。リース・ウィザースプーンとニコール・キッドマンが主演・製作。原作は女性の友情と家庭内暴力を描く社会派ミステリ。",
    recommendedFor: "サスペンスフルな女性ドラマが好きな方。上質な社会派ミステリ小説を読みたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "you-novel-overseas",
    mediaTitle: "YOU/ユー 〜君だけを見ている〜",
    mediaType: "overseas",
    mediaYear: 2018,
    originalExists: true,
    originalTitle: "ユー",
    originalAuthor: "キャロライン・ケプネス",
    originalType: "novel",
    adaptationLabel: "Netflixドラマ化",
    description: "書店員の男がSNSを駆使して女性に執着するダークサスペンス。Netflix世界的ヒットドラマ。",
    searchAliases: ["YOU ユー", "キャロライン・ケプネス", "ジョー・ゴールドバーグ", "Netflix"],
    adaptationNotes: "Lifetime放送後Netflixで配信し爆発的ヒット。原作小説は二人称（YOU視点）の独特な語りが特徴。シーズン4まで放送。原作シリーズは複数巻ある。",
    recommendedFor: "ダークで心理的なサスペンスが好きな方。ストーカーの視点から描かれる独特な語りを楽しみたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "outlander-novel-overseas",
    mediaTitle: "アウトランダー",
    mediaType: "overseas",
    mediaYear: 2014,
    originalExists: true,
    originalTitle: "アウトランダー",
    originalAuthor: "ダイアナ・ガバルドン",
    originalType: "novel",
    adaptationLabel: "Starzドラマ化",
    description: "第二次大戦後の看護師が18世紀スコットランドにタイムスリップするロマンティック歴史ファンタジードラマ。",
    searchAliases: ["アウトランダー", "ダイアナ・ガバルドン", "スコットランド", "タイムトラベル"],
    adaptationNotes: "Starzが7シーズン制作。原作は全9冊の長大シリーズ。歴史的な正確さとロマンスの融合が特徴。原作はスコットランドの歴史的背景の描写がより詳細。",
    recommendedFor: "歴史×ロマンス×ファンタジーが好きな方。壮大な歴史ロマン小説を読みたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "foundation-novel-overseas",
    mediaTitle: "ファウンデーション",
    mediaType: "overseas",
    mediaYear: 2021,
    originalExists: true,
    originalTitle: "銀河帝国の興亡（ファウンデーション）",
    originalAuthor: "アイザック・アシモフ",
    originalType: "novel",
    adaptationLabel: "Apple TV+ドラマ化",
    description: "銀河帝国の崩壊を予測した数学者が文明保存の財団を設立する壮大なSF叙事詩のドラマ化。",
    searchAliases: ["ファウンデーション", "アシモフ", "Apple TV", "銀河帝国"],
    adaptationNotes: "1951年刊行のSFクラシックをApple TV+でドラマ化。映像化困難とされた原作を大規模予算で実現。原作はハーリー・セルダンらの思想と歴史観が中心。",
    recommendedFor: "壮大なSF世界観が好きな方。アシモフのSF古典に触れたい人。ハードSFに挑戦したい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "wheel-of-time-novel-overseas",
    mediaTitle: "ウィール・オブ・タイム",
    mediaType: "overseas",
    mediaYear: 2021,
    originalExists: true,
    originalTitle: "時の車輪",
    originalAuthor: "ロバート・ジョーダン",
    originalType: "novel",
    adaptationLabel: "Amazon Prime Videoドラマ化",
    description: "魔法を使える女性「エイズ・セダイ」が選ばれし者を守りながら戦う壮大なファンタジードラマ。",
    searchAliases: ["ウィールオブタイム", "時の車輪", "ロバート・ジョーダン", "Amazon"],
    adaptationNotes: "全14巻の超長編ファンタジー小説をAmazonがドラマ化。ゲーム・オブ・スローンズに続く大型ファンタジーとして期待。原作はトールキンと並ぶ大作として評価が高い。",
    recommendedFor: "壮大なエピックファンタジーが好きな方。指輪物語のような長大なファンタジー世界に浸りたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "bosch-novel-overseas",
    mediaTitle: "BOSCH/ボッシュ",
    mediaType: "overseas",
    mediaYear: 2014,
    originalExists: true,
    originalTitle: "ボッシュ（ハリー・ボッシュシリーズ）",
    originalAuthor: "マイクル・コナリー",
    originalType: "novel",
    adaptationLabel: "Amazon Prime Videoドラマ化",
    description: "ロサンゼルス市警刑事ハリー・ボッシュの事件捜査を描く本格警察ドラマ。全7シーズン。",
    searchAliases: ["ボッシュ", "マイクル・コナリー", "Harry Bosch", "LAPD"],
    adaptationNotes: "Amazon Primeオリジナルとして7シーズン放送後、続編「ボッシュ：レガシー」も配信。原作はベストセラー警察小説シリーズ。ドラマは複数の原作エピソードを組み合わせて各シーズンを構成。",
    recommendedFor: "本格警察ミステリーが好きな方。マイクル・コナリーの職人的なクライムノベルを読みたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
  {
    id: "jack-reacher-novel-overseas",
    mediaTitle: "ジャック・リーチャー",
    mediaType: "overseas",
    mediaYear: 2022,
    originalExists: true,
    originalTitle: "ジャック・リーチャー（シリーズ）",
    originalAuthor: "リー・チャイルド",
    originalType: "novel",
    adaptationLabel: "Amazon Prime Videoドラマ化",
    description: "元米陸軍調査官の一匹狼リーチャーが各地の不正や犯罪に立ち向かうアクションサスペンスドラマ。",
    searchAliases: ["ジャックリーチャー", "リー・チャイルド", "Jack Reacher", "Amazon"],
    adaptationNotes: "トム・クルーズ主演の映画版に続きAmazonがドラマシリーズとして再始動。原作は「キリング・フロア」から始まる全27冊以上のベストセラーシリーズ。",
    recommendedFor: "ハードボイルドなアクションサスペンスが好きな方。一匹狼のヒーローが活躍するスリリングな物語を楽しみたい人に。",
    workId: "",
    thumbnailUrl: "",
  },
];

/** フィルタ定義 */
export const MEDIA_TYPE_FILTERS = [
  { value: "all" as const, label: "すべて" },
  { value: "movie" as const, label: "映画" },
  { value: "drama" as const, label: "ドラマ" },
  { value: "anime" as const, label: "アニメ" },
  { value: "overseas" as const, label: "海外" },
] as const;

export type FilterValue = (typeof MEDIA_TYPE_FILTERS)[number]["value"];

/** クイック検索チップ */
export const QUICK_SEARCH_CHIPS = [
  "鬼滅の刃",
  "呪術廻戦",
  "SPY×FAMILY",
  "葬送のフリーレン",
  "君の名は。",
  "半沢直樹",
  "逃げるは恥",
  "キングダム",
  "SLAM DUNK",
  "チェンソーマン",
  "三体",
  "ガリレオ",
];

/** メディア種別表示設定 */
export const MEDIA_TYPE_CONFIG: Record<
  MediaType,
  { label: string; icon: string; badgeClass: string }
> = {
  movie: {
    label: "映画",
    icon: "🎬",
    badgeClass: "bg-blue-100 text-blue-700",
  },
  drama: {
    label: "ドラマ",
    icon: "📺",
    badgeClass: "bg-purple-100 text-purple-700",
  },
  anime: {
    label: "アニメ",
    icon: "✨",
    badgeClass: "bg-amber-100 text-amber-700",
  },
  overseas: {
    label: "海外",
    icon: "🌍",
    badgeClass: "bg-emerald-100 text-emerald-700",
  },
};

/** 原作種別表示設定 */
export const ORIGINAL_TYPE_CONFIG: Record<
  OriginalType,
  { label: string; badgeClass: string }
> = {
  novel: { label: "小説", badgeClass: "bg-indigo-100 text-indigo-700" },
  manga: { label: "漫画", badgeClass: "bg-rose-100 text-rose-700" },
  nonfiction: {
    label: "ノンフィクション",
    badgeClass: "bg-stone-100 text-stone-600",
  },
  essay: { label: "エッセイ", badgeClass: "bg-stone-100 text-stone-600" },
};
